...
| **Causa** | **Diagnosticada (RF09-REACTIVACION · 21-09-2026): HTTP 408 intermitente por diseño síncrono, no "el disparo falla".** El síntoma original (ago-2026, `VP-2026-0053`) era de la era del fallo sistemático; la evidencia viva de `LogEscenarios` (sep 9–11) es **mayoría `✓ OK · HTTP 200`** con la cadena completa (webhook → extracción → propagación `AT03-Ext`). El residual: el blueprint v2.1 respondía con `WebhookRespond (200)` **al final** de cada rama (tras Dropbox `getFile` + Claude con `timeout:""`); cuando la latencia excedía el techo del `fetch` de Airtable Scripts, el trigger recibía 408 y marcaba `error`. Descartadas: URL (resuelve → hay 200s), firma HMAC (el hook `CustomWebHook` no la verifica → cosmético), content-type/payload, Secrets de Railway (el script usa Secrets de la Automation). |
| **Resolución** | **Fix aplicado (async v2.2).** Blueprint convertido a **asíncrono**: `WebhookRespond (200)` movido al **inicio** (módulo #26, tras el hook y antes de Dropbox/Claude) y quitados los 3 terminales (#17/#21/#24) — Make admite una sola respuesta por ejecución; el `estado_extraccion` final lo escriben los `UpdateRecords` internos, no el cuerpo HTTP. Además `timeout` explícito de `180` s en el módulo Claude (#10). Editado en `docs/_artefactos/make/SC-RF09-ExtraccionClaude.blueprint.json` (name bump v2.1→v2.2). **No se tocó `AT-RF09-Trigger_script.js`** (correcto, probado por los 200s). |
| **Dueño** | Sergio (reimport en Make) · Claude-Code (edición del blueprint) |
| **Fecha objetivo** | Resuelta en la tanda RF09-REACTIVACION (21-09-2026). |
| **Estado** | **cerrada — 408 timeout residual, resuelto por async v2.2.** ⚠ Cierre a nivel de edición del blueprint; queda pendiente el reimport manual de Sergio en Make + verificación E2E (FASE 2 · BLOQUE 3/4) para confirmar 0 × 408 en runtime. |
| **Origen** | Tanda 3 · paso 5, casos (a) y (b) · sesión del 06-ago-2026 · **cerrada en RF09-REACTIVACION 21-09-2026** |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §8 (nota de diseño) y §8.2 (campo `dropbox_path`) · afecta a la futura implementación del path de §8.1 |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` (8 apariciones, incluidas §5.2, §5.2.1 y el glosario) vs `docs/schema-airtable.md:51` |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §1.3.2 (bloque *Coordinación*), §1.3.3 (eventos de coordinación), §2.3 (RF-TAS-05), §2.11, §2.12 (declaración de la tabla) y §5.2 · vs base `app9G7lLkIV3CpeLa`, cuyo listado de **68 tablas no la contiene** |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.7 · vs `docs/_md/Imagenes_IF_Tasador_v4.pdf` p. 21 (Pantalla 4, partes 1 y 2) |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.8 · vs `docs/_md/Imagenes_IF_Tasador_v4.pdf` p. 22 (Pantalla 5, partes 1 y 2) |
...
| **Archivo:línea** | `docs/_md/Imagenes_IF_Tasador_v4.pdf` pp. 13 y 22 (`components/tasacion-form.tsx`, `IntentosIndicator`, `MAX_INTENTOS = 3` en `use-estado-tasador`) · vs `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2 (decisión capital 1) y §2.13 |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.10 (footer de acciones) · vs `docs/_md/Imagenes_IF_Tasador_v4.pdf` p. 27, punto 2 |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.10 (acción Confirmar) · vs `docs/_md/Imagenes_IF_Tasador_v4.pdf` pp. 29-30 |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.1 · vs `docs/_md/Imagenes_IF_Tasador_v4.pdf` p. 17 |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.4 y §2.13 · vs `docs/_md/Imagenes_IF_Tasador_v4.pdf` pp. 11-12 (árbol de rutas del App Router) y pp. 17-30 |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.2 y §2.12 (campo `horas_restantes`) · vs `docs/_md/Imagenes_IF_Tasador_v4.pdf` p. 17, punto 1.1, y §5.2.4 (RF-53) del propio spec |
...
| **Archivo:línea** | `docs/schema-airtable.md` §1 (*Dominio C_ · Configuración*, líneas 37-58) y `docs/_md/plan_ejecucion_UItasador_v1.4.md` §3.1 (ruta `GET /api/tasaciones/config/defaults`) · vs base `app9G7lLkIV3CpeLa`, verificada vía Meta API el 17-ago-2026 |
...
| **Archivo:línea** | `docs/schema-airtable.md` §1 (`TX_DocumentosGenerados` · `tbl5sYnGPZXgYCBSY`, documentada como *"No usada en IF-02"* y sin tabla de campos) · `docs/_md/plan_ejecucion_UItasador_v1.4.md` §10.1 (cabecera del preview: *"la versión del informe debe coincidir con la del registro vigente de `TX_DocumentosGenerados` para esa solicitud"*) · vs base `app9G7lLkIV3CpeLa`, verificada vía **MCP** el 18-ago-2026 |
...
| **Archivo:línea** | `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.12 (bloque *Campos nuevos en `TX_Solicitudes`*) · vs base `app9G7lLkIV3CpeLa`, `TX_Solicitudes.observacion_rechazo_tasador` = **`fldAccib5yNYaOmJc`**, `multilineText` |
...
| **Archivo:línea** | `components/tasador/coordinar-visita.tsx`, comentario del banner de reapertura retirado · vs `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.3 (**RF-TAS-04**) y §1.4 (excepción acotada a RN-59) |
...
| **Archivo:línea** | `lib/tasador/cola-filtros.ts` → `esPorCoordinar()` · vs `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §4.1 y **RF-TAS-01** |
...
| **Archivo:línea** | `lib/tasador/cola-filtros.ts` → `esPorCoordinar()` · vs `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` §2.1, **RF-TAS-01**, §2.2 · relacionada con **CI-021** y **CI-045** |
...
| **Archivo:línea** | `docs/_md/plan_ejecucion_UItasador_v1.4.md` §5 (P4-TAS) · vs el árbol real de código |
