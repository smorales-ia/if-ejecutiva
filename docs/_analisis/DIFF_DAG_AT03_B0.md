...
> ⚠ **Atribución de celda de F_ValorReposicionUF (D-1).** La reposición **no** se calcula literalmente como `CC70 + BI60`. La fórmula literal del xlsm es `BG72 = IF(AN61=0, CD37*0,8 + CC46, CD37 + CC46)`, numéricamente idéntica (`CD37=CC70=8.496,94` ; `CC46=BI60=750` → `9.246,94`). Las celdas `CC70`/`BI60` de la tabla son las fuentes de los sumandos, no la celda de salida.
