// AT03_Calculos (wflSBI7cjLNc0rkV5) — script LIVE / DEPLOYED
// Fuente: captura MCP get_automation 2026-09-23 (deploymentStatus=deployed · configurationStatus=valid),
//         re-materializada esta sesion desde AT03_Calculos_DAG_pre-T-MC-P0.js.
// NOTA: en esta sesion NO se re-extrajo por MCP (el server oficial mcp.airtable.com
//       no lee automations; la captura previa vino de otro MCP). Contenido identico
//       al pre-T-MC-P0 salvo line endings: aqui NORMALIZADO a LF para evitar el
//       falso-drift CRLF que bloqueo la sesion del 22-09. Rollback byte-fiel: usar
//       el pre-T-MC-P0 (CRLF). deploymentStatus=deployed al 2026-09-23.
// --------------------------------------------------------------------------------
// ============================================================
...
// b0-4 (v11.1.1): FIX smoke test — la transicion final estado='calculada' (paso 11)
//       pasaba un string pelado a un singleSelect; Airtable Scripting lo rechaza
//       ("Field fld2H2r0GMeVfNO26 cannot accept the provided value"). Es un bug
//       LATENTE identico en v11.0 (por eso el estado quedaba en 'visitada'), no lo
//       introdujo b0. Fix: forma canonica {name:'calculada'} + try/catch. El lector
//       del cuadro y la inyeccion al SCOPE quedan intactos.
//
...
// b0.1: singleSelect en forma canonica {name:...} (Airtable Scripting rechaza el
//       string pelado -> "Field ... cannot accept the provided value") y guardado
//       en try/catch como el resto de escrituras de select del script
//       (logEventoCompleto). Escritura NO fatal: si la transicion se rechaza, se
//       loguea y el script termina igual (AT04 tambien puede transitar el estado).
// ----------------------------------------------------------------
try {
