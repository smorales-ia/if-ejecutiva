...
>
> **v1.9.13** — sucede a `VProperty_Especificacion_Proyecto_v1_9_12.md`, que queda marcado SUPERSEDED.
...
Fase 2 · Análisis y Diseño · Documento maestro de requisitos
  ...
  **Versión**         1.9.13 · 21-ago-2026 · Incorpora la segunda tanda de
...
**Valor**: seis motivos · **Fuente**: audios `p1` y `p2` · **Estado**: pendiente de ratificación por el product owner (**A-25**).
...
> ⚠ **Los tres factores no aparecen en la plantilla operativa vigente (A-28).** La revisión del libro `Formato Informe VProperty Enero2026.xlsm` hecha para v1.9.13 buscó el origen de `factor_sup`, `factor_edad` y `factor_distancia` y no lo encontró: el cuadro de comparables calcula el valor unitario de forma directa, como `(total UF − UF/m² de terreno × superficie de terreno − obras complementarias) / superficie construida` `[Excel: Portada!AX29]`, sin ninguna columna de homogeneización. Lo que la plantilla sí tiene son **dos** factores multiplicativos, `D. F.` y `F. M.`, en el cuadro de valoración `[Excel: Portada!AX50 · BA50]`, ambos con valor por defecto `1` `[Excel: Portada!AX51:AX53 · BA51:BA53]`, aplicados como `F. M. × D. F. × UF/m² nuevo` `[Excel: Portada!BD51]`. Su nombre desarrollado no está escrito en el libro. En consecuencia **A-18 no se cierra con esta revisión**, y se abre **A-28** sobre si los tres factores de este documento describen la práctica real del negocio o si la práctica real son estos dos. Hasta resolverlo, la columna de factores de la grilla se conserva tal como está especificada y no se precarga con ningún valor.
...
Hasta v1.9.12 este conjunto no tenía origen declarado y **A-14** lo bloqueaba entero. La revisión de la plantilla operativa vigente cambia el estado del problema: **los valores existen, están completos y llevan años en producción** dentro de la hoja de antecedentes del libro `Formato Informe VProperty Enero2026.xlsm`. Lo que sigue abierto es dónde se alojan dentro del sistema, que es una decisión de arquitectura de datos y no una elicitación (**A-27**). El §2.8.1 que sigue especifica los valores; la tabla que los guarde se decide aparte.
...
| **RF-TAS-08** | **Valores por defecto parametrizados** *(defaults constructivos especificados en §2.8.1 · domicilio pendiente de A-27 · factores pendientes de A-18 y A-28)* |
|---|---|
| **Descripción** | Los factores de homogeneización, los coeficientes por defecto y los valores por defecto de características constructivas y terminaciones se precargan desde la capa de configuración vía API Route, no desde constantes del frontend. Los campos precargados muestran badge "Pre-llenado · editable" y el tasador puede sobrescribirlos. Los defaults constructivos y de terminaciones quedan **especificados valor por valor en §2.8.1**; la tabla de configuración que los albergue está pendiente (**A-27**), y hasta que exista ese subconjunto no se libera, pero deja de ser una elicitación abierta. Los factores de homogeneización siguen sin valor de referencia en ninguna tabla (**A-18**) y su propia composición está en duda (**A-28**). |
...
- **Etapa 2** — las 4 h del SLA ideal son la política interna de primer
  ...
  ocurre en TX_CoordinacionVisita (§2.3, §2.12). Estos dos umbrales miden
  el cumplimiento y **no son** el recordatorio automático al tasador ni el
  tope de respuesta al cliente, que son mecanismos distintos y se
  especifican en §5.2.8.
...
**Valor**: siete motivos · **Fuente**: audio `p3` · **Estado**: pendiente
de ratificación por el product owner (**A-26**).
...
**Valor del umbral de coordinación**: 8 horas hábiles desde la
asignación · **Fuente**: audios `p5` y `p7` · **Estado**: pendiente de
ratificación por el product owner (**A-22**). La misma tanda de audios
menciona 4, 6 y 24 horas en distintos momentos: las dos primeras son los
umbrales de la etapa 2 ya fijados en §5.2.4 y la última es el tope de
respuesta al cliente que se define abajo. El recordatorio es un cuarto
número y por eso se ratifica por separado.

**Canales.** Correo y WhatsApp. El segundo es petición explícita del
cliente —*"que le llegue un nuevo mail y un nuevo whatsapp"*— y **no
tiene proveedor definido** (**A-24**); mientras no lo tenga, el
recordatorio se emite sólo por correo y la parte de WhatsApp queda
declarada y no implementada. La notificación por WhatsApp al tasador
sigue registrada como fuera de alcance de v1.9 en §1.9 · FUT-EJ-10; esta
subsección no la introduce en alcance, fija su contrato para cuando
entre.
...
verde. Si se modela como umbral agregado sobre ese tramo o sólo como
corte del reporte de §5.2.9 está pendiente (**A-23**).
...
La conveniencia de incluir un grupo de día 0 —las visitas del propio día—
está planteada por el cliente y pendiente de definición (**A-32**).
...
reclama. Se especifica en dos superficies, que consultan lo mismo: un
...
SLA operacionales del negocio en v1.9.7, y D-17 a D-21, emergentes de la
segunda tanda de audios del cliente y de la revisión de la plantilla
operativa vigente en v1.9.13.
  ...
  **ID**     **Punto pendiente**                 **Responsable**    **Fecha
  ...
  D-17       Umbral del recordatorio automático  Héctor Martínez    30-sep-2026
  (v1.9.13)  de coordinación al tasador          (product owner)
             (§5.2.8). Los audios de agosto de
             2026 declaran 8 horas hábiles, y
             la misma tanda menciona 4, 6 y 24
             en otros pasajes. Los dos primeros
             son los umbrales de la etapa 2 y
             el último es el tope de respuesta
             al cliente. Falta ratificar el
             cuarto. Registrada como A-22.
             Impacto: §5.2.8. Abierta.

  D-18       Modelado del tope de 24 horas para  Héctor Martínez +  30-sep-2026
  (v1.9.13)  responder al cliente con fecha de   Arquitecto de
             visita (§5.2.8): umbral agregado    Software
             sobre las etapas 2 a 4, o sólo
             corte del reporte de §5.2.9. El
             compromiso de servicio no está en
             duda; su realización sí. Registrada
             como A-23. Impacto: §5.2.4, §5.2.8,
             §5.2.9. Abierta.

  D-19       Proveedor y contrato del canal      Héctor + Ingeniero 31-oct-2026
  (v1.9.13)  WhatsApp para los recordatorios al  Make
             tasador (§5.2.8). El cliente lo
             pide de forma explícita; no hay
             proveedor, plantilla ni costo
             definidos, y §1.9 · FUT-EJ-10 lo
             mantiene fuera de alcance de v1.9.
             Registrada como A-24. Impacto:
             §5.2.8, §5.3. Abierta.

  D-20       Domicilio de los defaults           Arquitecto de      30-sep-2026
  (v1.9.13)  constructivos de §2.8.1: tabla      Datos + Héctor
             nueva de catálogo, y si la clave
             es global, por tipo de propiedad o
             por tipo × estado de uso, que es
             como los ramifica la plantilla
             vigente. Los valores ya están
             especificados; falta dónde viven.
             Crear tabla en Airtable exige
             aprobación explícita. Registrada
             como A-27. Impacto: §2.8, §2.8.1,
             RF-TAS-08. Abierta.

  D-21       Composición real de los factores    Héctor + Visador   31-oct-2026
  (v1.9.13)  de homogeneización (§2.8). La       titular
             plantilla operativa vigente no
             contiene factor_sup, factor_edad
             ni factor_distancia: aplica dos
             factores, D. F. y F. M., ambos con
             default 1. Falta determinar si los
             tres de este documento describen
             la práctica real. Registrada como
             A-28, ligada a A-18. Impacto:
             §2.8, RF-TAS-08. Abierta.
  -------------------------------------------------------------------------------
