...
>
> **v2.7 — sucede a `VProperty_Motor_Calculo_AT01_AT10_v2_6.md`** (P13-TAS · 07-sep-2026), que queda
> en su sitio como versión anterior. Cambio: el **Paso 1 del DAG** (`uf_m2_promedio`) se deja
> consistente con el flujo real —**promedio simple** de la muestra, **sin factores de
> homogeneización**—. El cuadro que el tasador fotografía no trae `factor_*` (**A-44**) y P13-TAS los
> sacó del modelo de IF-03 (**R-COMP-1**); `lib/tasador/lectura-informe.ts` ya calcula el promedio con
> la fórmula directa `[Excel: Portada!AX29]`, sin coeficientes. El detalle vive en el changelog al pie
> y en `docs/_sync_ifTasador_v1/SYNC_LOG.md`.
...
> ────────────────────────────────────
>
> 1 uf_m2_promedio (promedio simple) (comparables de la muestra · sin factores homolog.)
>
...
## Changelog — v2.7 · P13-TAS · 07-sep-2026

Bump de versión (copia + bump, R-COMP-4): sucede a v2.6, que queda en su sitio.

| # | Cambio | Justificación |
|---|---|---|
| 1 | **Paso 1 del DAG** (`uf_m2_promedio`): de `(homologado) (5 comparables + factores homolog.)` a **`(promedio simple) (comparables de la muestra · sin factores homolog.)`** | El cuadro fotografiado no trae `factor_*` (A-44); P13-TAS los saca del modelo de IF-03 (R-COMP-1). `lib/tasador/lectura-informe.ts` ya promedia con la fórmula directa `[Excel: Portada!AX29]`, sin coeficientes. Consistencia UI ↔ motor ↔ Origen de Datos v1.2. |
...
de Fase 3, recomendación EA). El documento era, hasta este cambio, **v2.6**.
