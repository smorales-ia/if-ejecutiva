# Plan de Ejecución IF-03 · UI Tasador v1.5 — Guía maestra para Claude Code

> **Versión del plan: v1.5** (07-sep-2026). Cambio respecto de v1.4: **P13-TAS · la sección D de
> comparables espeja el cuadro `[Excel: Portada!B28:AX44]` cuadro-a-cuadro**. La grilla pasa de una
> tabla plana con un único promedio a **dos bloques** (REF. OFERTAS · REF. C.B.R.), cada uno con sus
> columnas crudas de la foto (Fecha, Total UF, Sup. Terreno, Sup. Constr., OO.CC., UF/m² T., UF/m² C.)
> y **tres renglones de resumen** (`PROMEDIO DE LA MUESTRA`, `TASACION`, `TASACION V/S PROMEDIO DE LA
> MUESTRA`, promedios simples sin homogeneización). Se retira `comuna` del grid (el cuadro no la trae;
> sigue viva en el modelo · R-COMP-6) y el UF/m² pasa de calculado a **crudo de la fuente**. Los
> factores `factor_*` salen por completo del modelo de IF-03 (R-COMP-1). §8 sección D actualizada.
> Sin tandas nuevas, sin rutas nuevas. Referencia normativa reapuntada a
> `VProperty_Especificacion_Proyecto_v1_9_17.md`.
>
...
>
> **1 · A-27 cerrada: los defaults se particionan por tipo de propiedad × estado de uso.** Con la
> clave decidida, la sección E de P7-TAS pasa de *"con campos y sin valores"* a **implementable
> con precarga**. **A-14 queda cerrada del todo.** Lo único que la separa de producción es crear
> la tabla, que es tanda de schema con aprobación propia.
>
> **2 · A-28 cerrada: RF-TAS-08 ratificado con sus tres factores.** `factor_sup`, `factor_edad` y
> `factor_distancia` se usan en la práctica; no hay variante de dos factores. `D. F.` y `F. M.`
> quedan como observación (**A-35**, no bloqueante).
>
> **3 · A-22 cerrada: 4 horas hábiles**, coincidentes con el ámbar de la etapa 2. **No cambia
> nada en IF-03** — ver punto 5.
>
> **4 · A-18 sigue abierta y sigue bloqueando `GET /api/tasaciones/config/defaults`.** Ratificar
> *qué* factores no dice *cuánto* vale cada uno: `C_FactoresHomogeneizacion.valor_referencia`
> sigue vacío en sus 15 filas. Es ahora su **única** pregunta bloqueante.
>
> **5 · El umbral de 4 h no se replica en IF-03, y ahora hay un argumento más fuerte que la
> Regla T-A.** El chip "Por coordinar" ya dropeó su cota de 4 h por **CI-048** (re-etiquetada
> **A-36**), y reintroducirla para "cuadrar" con el recordatorio reabriría **CI-021** —recalcular
> horas hábiles en el cliente— y volvería a esconder las coordinaciones vencidas, que son las más
> urgentes. La Verificación 4 de P10-TAS se mantiene como **coherencia observada**, no como
> aritmética replicada.
>
> **v1.1** (21-ago-2026). Cambio respecto de v1.0: se incorpora la **segunda
> tanda de audios del cliente** y la revisión de la **plantilla operativa vigente**
> `Formato Informe VProperty Enero2026.xlsm`. Sin tandas nuevas, sin rutas nuevas y sin cambios
> en el orden de ejecución. Lo que cambia es el **estado de tres ambigüedades** y, con ellas, el
> alcance de dos tandas.
>
> **1 · A-14 deja de bloquear la sección E de P7-TAS.** La pregunta que A-14 hacía era dónde
> viven los defaults constructivos, y la respuesta implícita era que no existían. Existen: llevan
> años en producción dentro de la hoja de antecedentes de la plantilla vigente, y quedan
> especificados valor por valor en spec §2.8.1, con su celda de origen. A-14 **se reduce** a su
> mitad restante —qué tabla de Airtable los aloja— y esa mitad se renumera como **A-27**. La
> sección E pasa de "sin precarga" a "con precarga, condicionada a que exista la tabla".
>
> **2 · El catálogo de motivos de coordinación pasa de cuatro a seis valores** (spec §2.3 ·
> RF-TAS-12 · A-25). Afecta el `singleSelect` que P0.5-TAS crea y la UI que P4-TAS construye.
>
> **3 · A-18 no se cierra, y además se agrieta.** La plantilla vigente **no contiene**
> `factor_sup`, `factor_edad` ni `factor_distancia`: aplica otros dos factores, ambos con default
> `1`. Queda abierto si los tres factores de la spec describen la práctica real del negocio
> (**A-28**). `GET /api/tasaciones/config/defaults` sigue sin construirse.
>
> **4 · P10-TAS gana una verificación.** El recordatorio automático al tasador es un mecanismo
> nuevo del control de SLA (spec §5.2.8) y su umbral llega sin ratificar.
>
> **v1.0** (17-ago-2026). Primera emisión. Gobierna la construcción de
...
>
> **Precedencia.** Ante contradicción con `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md`
...
1. `docs/_md/plan_ejecucion_UItasador_v1.5.md` (este archivo, completo)
...
4. `docs/_md/VProperty_Especificacion_Proyecto_v1_9_17.md` — **§2 completa** (§2.1 a §2.16),
...
| **A-17** | Si el catálogo de motivos de contacto no logrado es paramétrico o fijo. | `TX_CoordinacionVisita.motivo` se crea como **`singleSelect` con los seis valores del catálogo cerrado** (ampliado de cuatro a seis en spec v1.9.13 · ver **A-25**), que es la realización que §2.12 declara. Si el negocio lo quiere paramétrico, migra a Link después. La UI lee el catálogo desde el API, **no** desde un enum hardcodeado en el cliente, de modo que la migración no toque la UI. | **P0.5-TAS** (schema) · **P4-TAS** (UI) |
...
- Despliega **"Motivo"** (obligatorio, catálogo cerrado de **seis valores** leído del API · A-17 · A-25) y **"Detalle"**
...
- Grilla **tabular densa**, no formulario acordeón. Una fila por comparable, columnas por
  atributo. **Ningún campo es editable.** La grilla se parte en **dos bloques** —**REF. OFERTAS** y
  **REF. C.B.R.**—, discriminados por `tipo_referencia`; sólo se pinta el bloque con al menos una fila.
- **Header fijo y scroll horizontal en móvil**, con la primera columna (N° · dirección) **sticky**.
  ...
  horizontal (R9).
- Orden de columnas, espejando el cuadro de origen (P13-TAS): N° · dirección (sticky),
  `fecha_publicacion`, `anio`, columna de contacto, `precio_uf` (Total UF), `sup_terreno_m2`,
  `sup_construccion_m2`, `oo_cc_uf` (OO.CC.), `uf_m2_terreno_f` (UF/m² T.), `uf_m2_construccion_f`
  (UF/m² C.). Los dos UF/m² se muestran **crudos de la foto**, no recalculados como `precio / sup`
  —el cuadro los obtiene con la fórmula directa `[Excel: Portada!AX29]`—. **`comuna` sale del grid**
  (el cuadro no la trae; el atributo sigue vivo y vigente, lo consume el informe · R-COMP-6).
  **Sin columnas de `factor_sup`, `factor_edad` ni `factor_distancia`** (R-COMP-1 · A-18 · A-44).
- Columna de contacto condicional por bloque: en **REF. OFERTAS** se muestra `telefono_contacto`;
  en **REF. C.B.R.**, `foja` y `numero` unidas como `foja-numero`.
- **Tres renglones de resumen por bloque**: `PROMEDIO DE LA MUESTRA` (promedio **simple** columna a
  columna, sin homogeneización, derivado en la UI), `TASACION` (valores del inmueble sujeto que
  produce el motor AT03 — la UI los muestra, no los calcula; se pintan «—» mientras `InformeData` no
  los traiga) y `TASACION V/S PROMEDIO DE LA MUESTRA` (`(tasacion − promedio) / promedio` sobre UF/m²
  C., «—» sin base).
- **No hay botón "Agregar comparable" ni acción de eliminar por fila.** El `POST` y el `DELETE` de
  `app/api/tasaciones/[id]/comparables/route.ts` **ya se retiraron** (CI-056); sólo queda el `GET`,
  que se conserva como superficie HTTP de la sección aunque la pantalla se hidrate server-side.
...
- El módulo de factores de homogeneización **ya no tiene consumidor** y se purgó en CI-056; su
  sucesor es `lib/tasador/comparables.ts` (aritmética de resúmenes, sin factores). Ver **CI-031** y **OV-6**.
...
2. **Defaults de características constructivas y terminaciones** (sección E: materialidad y estado
   ...
   **Especificados en spec §2.8.1 (RF-TAS-23) desde v1.1 de este plan.** La sección E ya **no** se
   construye a ciegas: los campos, sus catálogos cerrados y su valor por defecto están declarados
   uno a uno, con la celda de la plantilla operativa de la que salen. Lo que sigue pendiente es
   **dónde se alojan** (**A-27**), no cuáles son.

   *Qué construye P7-TAS (v1.2 · A-27 cerrada):* los campos de la sección E **contra los
   catálogos de §2.8.1** —36 valores de estructura soportante
   `[Excel: Antecedentes!BZ45:BZ80]`, 19 de cubierta `[Excel: Antecedentes!CE45:CE63]`, 13 de
   muebles de cocina `[Excel: Antecedentes!CO44:CO56]`, 11 de ventanas
   `[Excel: Antecedentes!BX54:BX64]`, y los demás que §2.8.1 enumera— con el badge
   ...
   *Comportamiento con clave sin fila:* una combinación de (tipo, estado de uso) que no tenga fila
   ...
   conjunto por defecto — spec §2.8.1: heredar produciría valores plausibles y falsos, que es el
   modo de fallo que el pre-llenado viene a evitar.

   *Lo que no cambia:* **sigue prohibido hardcodear.** Que ahora exista tabla y clave no autoriza
   a escribir los valores en el frontend; al contrario, es cuando más tienta, porque el dato ya
   se conoce y la tabla todavía no está. El criterio de aceptación conserva su `grep`.

   *Dos detalles de §2.8.1 que la UI debe respetar:* el atributo **material o marca** de cada
   recinto es **derivado del pavimento**, no capturable `[Excel: Antecedentes!W46:W50]`, y se
   renderiza como campo calculado de sólo lectura; y la **orientación de la construcción** se
   presenta **vacía**, porque el valor de la plantilla es un literal sin ramificación
   `[Excel: Antecedentes!Z17]` que §2.8.1 descarta como residuo.
...
├── seccion-e-terminaciones.tsx   # Catálogos de §2.8.1 · precarga por tipo × estado_uso
...
- [ ] **Precarga por clave:** dos solicitudes que difieran sólo en el tipo de propiedad reciben
      defaults distintos en cubierta y adosamiento; dos que difieran sólo en el estado de uso,
      estados de conservación distintos. Una combinación sin fila cargada deja los campos
      **vacíos**, sin heredar de ninguna otra.
- [ ] **Nada hardcodeado, con tabla o sin ella:** `grep -rn` de los literales de §2.8.1
      —`HORMIGON ARMADO`, `ACERO VOLCANITA`, `PISO FLOTANTE`, `MELAMINA Y POSTFORMADO`,
      `ENLUCIDO / PINTURA`— sobre `components/tasador/` y `lib/tasador/` devuelve **cero**. Que
      los valores se conozcan y la tabla pueda no estar todavía es exactamente lo que vuelve
...
> **Ambigüedades: A-22 cerrada** (umbral de 4 h hábiles, coincidente con el ámbar de e2) · **A-36** (la cota del chip, cerrada y aplicada).

> **CI-021 dejó de ser teórica (v1.1).** Las pruebas del cliente sobre el sistema, en agosto de
> 2026, reportan el síntoma exacto: al ingresar una solicitud la interfaz muestra días —*"apareció
> que quedaban dos horas y dos días"*— donde corresponden las horas de la etapa vigente. No es un
> hallazgo nuevo ni abre inconsistencia aparte; confirma que CI-021 es visible para el usuario
> final y sube su prioridad. La Verificación 2 de esta tanda es la que lo cierra del lado de
> IF-03.
...
**Verificación 4 · Coherencia con los recordatorios al tasador (spec §5.2.8 · A-22 cerrada).** El
control de SLA incorpora una segunda familia de avisos, dirigida al **ejecutor** y no al
responsable de área: el tasador recibe un recordatorio automático a las **4 h hábiles** sin
coordinar —el mismo instante en que la etapa 2 pasa a ámbar— y otro a las 24 h de la visita sin
informe. **IF-03 no los dispara** —son de IF-02 · AT08— pero su UI no puede contradecirlos: un
tasador que acaba de recibir "te falta coordinar" y abre la app con la solicitud fuera del chip
"Por coordinar" concluye, con razón, que uno de los dos miente.

- Verificar que una solicitud que califica para el recordatorio de coordinación **sigue visible**
  en el chip "Por coordinar", o —si su reloj se detuvo por coordinación rechazada— muestra el badge
  "Esperando contacto de ejecutiva", que es la explicación de por qué salió.
- Verificar que el badge de SLA de la card **no dice "En plazo"** sobre una solicitud cuyo
  recordatorio ya se emitió. Con el umbral ratificado esto es comprobable de forma exacta: a las
  4 h hábiles la etapa 2 está en ámbar, y el badge debe reflejarlo.

> **El umbral está ratificado y aun así NO se replica en IF-03.** v1.1 lo justificaba por la
> Regla T-A y porque el número llegaba sin ratificar. Ratificado, el argumento **es más fuerte, no
> más débil**, y conviene dejarlo escrito porque la tentación de "cuadrar" ahora es mayor:
>
> 1. **Reintroducir la cota de 4 h en el chip reabriría CI-021.** Calcular "¿pasaron 4 h hábiles?"
>    en IF-03 exige recorrer la ventana de §5.2.1 y los feriados en el cliente, que es exactamente
>    la aritmética que CI-021 retiró al eliminar `horas_restantes` por producir una cifra que
>    ningún otro punto del sistema podía reproducir.
> 2. **Y desharía A-36.** La cota se dropeó de la **pertenencia** del chip precisamente porque
>    esconder las coordinaciones vencidas —las de más de 4 h, las más urgentes— vacía de sentido a
>    RF-TAS-01. Un recordatorio que se emite a las 4 h y un chip que expulsa a la solicitud a las
>    4 h se contradicen entre sí, no se complementan.
> 3. **La urgencia ya se lee donde corresponde:** en el **orden** por `sla_etapa_vence_ts` y en el
>    color del badge, ambos calculados por el motor.
>
> Esta verificación comprueba **coherencia observada**, no aritmética replicada. Si el umbral
> cambiara en `C_SLA_Etapas`, IF-03 no se toca.

**Verificación 5 · Frontera R5 sobre todo el trabajo acumulado.** `git diff --stat` desde el inicio
...
cinco verificaciones encuentren mal: sustituir una reimplementación por el import correcto,
corregir el consumo del SLA, arreglar el filtro de `A_Cambios`. **No se construye funcionalidad
nueva**, y en particular esta tanda **no implementa** los recordatorios de la Verificación 4: los
observa.

### §11.2 Construcción — Pasos para Claude Code

1. Ejecutar las verificaciones 1 y 5 (son puro `grep` / `git`) y **listar hallazgos antes de tocar
   nada**.
...
5-bis. Ejecutar la verificación 4 sobre una solicitud que califique para el recordatorio de
   coordinación, y confirmar que la cola y el badge no la contradicen. Si divergen, **el ajuste va
   del lado de IF-03**: la lógica del recordatorio es de IF-02 y no se replica acá.
...
- [ ] **Coherencia con los recordatorios (A-22):** una solicitud que califica para el recordatorio
      de coordinación sigue en el chip "Por coordinar", o explica su ausencia con el badge
      "Esperando contacto de ejecutiva". Su badge de SLA **no** dice "En plazo".
- [ ] El umbral de 4 h **no aparece** en el código de IF-03, en ninguna forma, pese a estar
      ratificado: su domicilio es `C_SLA_Etapas` y su lectura es el semáforo que el motor emite.
      El candado de `cola-filtros.test.ts` —la solicitud sin coordinar de hace un mes que sigue en
      el chip— debe seguir verde (**A-36**).
...
valor por defecto de cada factor · el umbral de 4 h **no se replica** en IF-03, con argumento
reforzado en §11.1 · **A-35** nueva por `D. F.`/`F. M.` · sin tandas, rutas ni cambios de orden
nuevos) · v1.1 (segunda tanda de audios del cliente y
revisión de la plantilla operativa vigente · **A-14 reducida**: los defaults constructivos existen
y quedan especificados en spec §2.8.1 con su celda de origen, y sólo su domicilio sigue abierto
como **A-27** · catálogo de motivos de coordinación de cuatro a **seis** valores, A-25 ·
**A-28** nueva: la plantilla no contiene los tres factores de homogeneización de RF-TAS-08 y
aplica otros dos, de modo que A-18 no se cierra · **A-22** nueva: umbral del recordatorio al
tasador, declarado y sin ratificar · P10-TAS gana la Verificación 4 de coherencia con los
recordatorios · CI-021 confirmada en uso por el cliente · sin tandas, rutas ni cambios de orden
nuevos) · v1.0 (primera emisión · 14 tandas P0-TAS a
...
listados sin editar en §16 · riesgos conocidos en §17) · Base: Especificación v1.9.14 §2 ·
Patrón: `docs/_md/plan-ejecucion-if02-v1_9.md` v1.14*
