...
aparecen porque no transicionan solicitudes.
  ...
  **Interfaz**   **Acción de UI** **Transición de         **Automatización en
  ...
  IF-02          Asignar Tasador  creada → asignada       ⚠ v1.9 (REGLA A):
                 (botón, sin AT02)                       acción manual única
                                                          de la Ejecutiva ---
                                                          no hay "Pasar a
                                                          asignada" separado
                                                          ni validación AT02
...
(mensaje humano · ver sección 6).
  ...
  **Botón /       **Interfaz**   **Precondiciones de          **Si falla
  ...
  Asignar         IF-02          ⚠ v1.9 (REGLA A): botón       El botón se
  Tasador                        único "Asignar Tasador" ---   deshabilita e
                                 requiere                      indica qué
                                 datosMinimosFaltantes vacío.  datos faltan.
                                 Visible sólo sin tasador y en
                                 estados no cancelada/cerrada;
...
Diseño de Capa de Datos v2.5.
  ...
  **Punto de         **Regla (origen oficial)**            **Mensaje humano**
  ...
  IF-02 · Asignar    ⚠ v1.9 (RN-44/REGLA A):               El botón se deshabilita y
  Tasador            datosMinimosFaltantes vacío ---        muestra qué datos faltan
                     reemplaza la precondición vieja de     para poder asignar (ya no
                     tasador+visador+fecha_visita           es el mensaje fijo de
                     (RN-09, superseded)                    tasador/visador/fecha)

  IF-02 · Crear      ⚠ v1.9 (REGLA B): ningún campo con      Toast con N° de campos +
  solicitud          error (incl. bloques repetibles         detalle de los primeros +
  (validación)       Unidades/Contactos); N° de operación    Alert destructivo al
                     único (conflicto de negocio, no error   inicio del formulario
                     de formulario)                          listando todos los
                                                              campos con problema.
...
**Wizard de creación (v1.9 · 3 fases)**

1. **Fase 1 · Modo de creación** --- documentos (carga inicial de
   respaldos) o manual.
2. **Fase 2 · Tipo de propiedad** --- Nuevo / Usado. Es el interruptor
   que determina qué campos y bloques se muestran en el resto del
   flujo (RN-45/RN-49/RN-50).
3. **Fase 3 · Formulario** --- adaptado según las dos fases anteriores.

**Formulario de detalle (v1.9 · 4 secciones)**

  ------------------------------------------------------------------------
  **Sección**   **Contenido**
  ------------- ----------------------------------------------------------
  A · Origen    Ejec. Comercializador y Ejec. Formalizador (roles
                separados), tipo de cliente de origen, bloque repetible
                de Contactos de visita.

  B ·           Tabla de N Unidades: tipo de bien, rol SII, superficies
  Propiedad     con origen declarado (RN-45) y adjunto de respaldo
                obligatorio por unidad.

  C · Personas  Comprador y Vendedor, cada uno con RUT.
  de la
  operación

  D · Producto  Producto, banco financista condicional, observaciones.
  ------------------------------------------------------------------------

Documentos y Adjuntos **salen** del formulario de creación y pasan a
vivir en el botón "Documentos y Adjuntos" del detalle (ver más abajo) ---
siempre visible tras crear la solicitud, ya no es una sección del
wizard.
...
- **REGLA A --- Asignación de tasador (v1.9, reemplaza el override
  automático de AT02):** la solicitud se crea sin tasador
  (estado `creada`, "Sin asignar"). Sólo existe el botón "Asignar
  Tasador" --- no existe flujo de "Reasignación". El botón aparece
  únicamente cuando la solicitud no tiene tasador y el estado lo
  permite (no `cancelada`, no `cerrada`); una vez asignado, el botón
  **desaparece**. Requiere `datosMinimosFaltantes` vacío --- si faltan
  datos, el botón se deshabilita indicando cuáles. Al confirmar: fija
  el tasador, transiciona `creada → asignada`, registra
  `fecha_asignacion`, marca el correo como enviado y agrega dos
  eventos al historial (correo de asignación + asignación manual).
  Mientras la solicitud siga en `creada`, la Ejecutiva puede cambiar
  el tasador ya asignado desde "Editar solicitud" (REGLA C) --- eso
  reemplaza cualquier necesidad de "reasignar".

- **REGLA C --- Edición (RN-59):** editable sólo en estado `creada`
  (botón "Editar solicitud" visible únicamente ahí). En cualquier otro
  estado con tasador asignado, todos los datos quedan en modo consulta
  (solo lectura) --- no depende solo de si hay tasador, depende de
  ambas condiciones.

**AT02 --- fuera de alcance de IF-02 en v1.9:** el Airtable Script
AT02 (selección algorítmica del tasador por zona/carga) permanece en
el catálogo de automatizaciones pero **no se invoca** desde el flujo
de IF-02. Ha sido reemplazado por la asignación manual descrita en la
REGLA A.
...
**Automatizaciones y transición de estado**
  ...
  **Acción de UI**     **Transición**     **Automatización**
  -------------------- ------------------ --------------------------------
  Crear solicitud      ∅ → creada         AT01 corre en Airtable al
  (alta interna)                          insertarse la fila con
                                          origen_canal=ingreso_manual.
                                          AT02 NO corre (v1.9, REGLA A) ---
                                          la solicitud queda sin tasador.

  Asignar tasador      creada → asignada  Acción manual de la Ejecutiva
  (botón "Asignar                         (REGLA A, sin AT02) · SC05
  Tasador")                               (Make → Gmail) notifica al
                                          tasador con link a IF-03.
...
ningún campo de la UI carece de origen en la capa de datos.
  ...
  **IF /    **Tablas Airtable**               **Automatización**   **Workflow / etapa** **Reglas de negocio**
  ...
  IF-02 /   TX_Solicitudes (W) ·              AT01 · SC05          Recepción → asignada Asignación manual
  F2        M_Tasadores/Visadores (R) ·       (AT02 fuera de                            única vía botón
            A_Eventos (W)                     alcance, v1.9)                            "Asignar Tasador"
                                                                                         (REGLA A) --- no hay
                                                                                         selección automática
                                                                                         por zona+carga desde
                                                                                         IF-02.
