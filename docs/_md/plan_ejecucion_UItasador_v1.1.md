# Plan de Ejecución IF-03 · UI Tasador v1.1 — Guía maestra para Claude Code

> **Versión del plan: v1.1** (21-ago-2026). Cambio respecto de v1.0: se incorpora la **segunda
...
>
> **Precedencia.** Ante contradicción con `docs/_md/VProperty_Especificacion_Proyecto_v1_9_13.md`
...
1. `docs/_md/plan_ejecucion_UItasador_v1.1.md` (este archivo, completo)
...
4. `docs/_md/VProperty_Especificacion_Proyecto_v1_9_13.md` — **§2 completa** (§2.1 a §2.16),
...
8. `docs/_sync_ifTasador_v1/gap/_ambiguedades.md` — **A-12 a A-18** y **A-22 a A-29** (las abiertas que tocan a IF-03)
9. `CLAUDE.md`
...
| **A-14** | ~~Qué tabla de configuración alberga los defaults constructivos.~~ **Reducida en v1.1.** Su mitad de contenido está resuelta: los defaults existen, llevan años en producción en la plantilla operativa y quedan especificados valor por valor en spec **§2.8.1** con su celda de origen. Lo que sigue abierto es sólo el domicilio, renumerado como **A-27**. | La sección E deja de construirse a ciegas: los campos, sus catálogos y sus valores por defecto son conocidos y se construyen contra ellos. La **precarga** queda condicionada a A-27. Sigue prohibido hardcodear: los valores de §2.8.1 son datos y se cargan como tales, nunca como constantes del frontend. | **P7-TAS** |
| **A-27** | Qué tabla de Airtable aloja los defaults constructivos de spec §2.8.1, y con qué clave: global, por tipo de propiedad, o por tipo × estado de uso, que es como la plantilla los ramifica `[Excel: FICHA SOLIC!K35 · K36]`. Crear tabla exige aprobación explícita (D-20). | P7-TAS construye la sección E **con los campos y catálogos de §2.8.1** y deja el punto de consumo de la precarga aislado en el mismo módulo que consume los factores. Mientras A-27 no cierre, la sección se libera **sin precarga**, no sin campos. | **P0.5-TAS** (tabla) · **P7-TAS** (consumo) |
...
| **A-22** | Umbral del recordatorio automático de coordinación al tasador. Spec §5.2.8 lo declara en **8 horas hábiles** desde la asignación, con estado *pendiente de ratificación*; la misma tanda de audios menciona 4, 6 y 24 en otros pasajes (D-17). | IF-03 **no implementa el disparo** —es de IF-02 · AT08— pero sí su efecto visible: el chip "Por coordinar" y el badge de la card no pueden contradecir al recordatorio que el tasador acaba de recibir. **P10-TAS verifica** la coherencia. Ningún umbral se escribe en el código de IF-03 (Regla T-A). | **P10-TAS** |
...
| **A-28** | Si los tres factores de homogeneización de RF-TAS-08 (`factor_sup`, `factor_edad`, `factor_distancia`) describen la práctica real. La plantilla operativa vigente **no los contiene**: aplica dos factores, `D. F.` y `F. M.`, ambos con default `1` `[Excel: Portada!AX50 · BA50]`, y calcula el valor unitario de los comparables sin homogeneizar `[Excel: Portada!AX29]`. Ligada a **A-18**. | Refuerza el diferimiento que A-18 ya impone: `GET /api/tasaciones/config/defaults` **sigue sin construirse** en P2-TAS. La grilla de comparables conserva sus tres columnas de factor como campos capturables **sin precarga**. No se implementa ninguna variante de dos factores mientras A-28 no cierre: sería sustituir una suposición por otra. | **P2-TAS** (no construye) · **P7-TAS** (consumidora) |
...
> **Ambigüedades e inconsistencias declaradas:** **A-13** (origen de los comparables) · **A-27**
> (dónde se alojan los defaults constructivos — sucede a **A-14**, cuya mitad de contenido quedó
> resuelta en spec §2.8.1) · **A-18** y **A-28** (los factores de homogeneización, sin valor de
> referencia y con su propia composición en duda) · **CI-014** (ocho secciones, no siete) ·
> **CI-015** (traza legacy del contador de intentos). **Regla T-B** es el criterio dominante de
> esta pantalla.
...
2. **Defaults de características constructivas y terminaciones** (sección E: materialidad y estado
   ...
   *Qué construye P7-TAS:* los campos de la sección E **contra los catálogos de §2.8.1** —36
   valores de estructura soportante `[Excel: Antecedentes!BZ45:BZ80]`, 19 de cubierta
   `[Excel: Antecedentes!CE45:CE63]`, 13 de muebles de cocina `[Excel: Antecedentes!CO44:CO56]`,
   11 de ventanas `[Excel: Antecedentes!BX54:BX64]`, y los demás que §2.8.1 enumera—, con el
   badge **"Pre-llenado · editable"** ya cableado y el punto de consumo aislado en el mismo módulo
   que consume el conjunto 1.

   *Qué NO construye:* la precarga efectiva, mientras A-27 no cierre. La sección se libera **con
   campos y sin valores**, que es distinto de liberarla sin campos. **Sigue prohibido
   hardcodear**: los valores de §2.8.1 son datos —incluido el hecho de que ramifican por tipo de
   propiedad y estado de uso— y se cargan como tales. Escribirlos en un `const` del frontend
   porque ahora se conocen sería exactamente el error que RF-TAS-08 previene, y el más fácil de
   cometer en esta tanda.
...
├── seccion-e-terminaciones.tsx   # Catálogos de §2.8.1; precarga pendiente de A-27
...
7. Construir la sección E **contra los catálogos de spec §2.8.1**, con el badge "Pre-llenado ·
   editable" cableado y el punto de consumo aislado. **No** precargar valores mientras A-27 siga
   abierta, y **no** precargar el conjunto 1 mientras A-18/A-28 sigan abiertas: la ruta
   `GET /api/tasaciones/config/defaults` no existe en este plan. Material o marca de cada recinto
   se renderiza como campo **derivado del pavimento** y de sólo lectura; orientación de la
   construcción se renderiza **vacía**.
...
- [ ] **A-27 respetado:** la sección E expone los campos y catálogos de spec §2.8.1 pero **no**
      precarga valores, y **no** los hardcodea. Verificable: `grep -rn` de los literales de
      §2.8.1 —`HORMIGON ARMADO`, `ACERO VOLCANITA`, `PISO FLOTANTE`, `MELAMINA Y POSTFORMADO`,
      `ENLUCIDO / PINTURA`— sobre `components/tasador/` y `lib/tasador/` devuelve **cero**. Que
      los valores ahora se conozcan es exactamente lo que vuelve tentador escribirlos; el criterio
      existe para atrapar eso.
...
> **Ambigüedad declarada: A-22** (umbral del recordatorio de coordinación).
...
**Verificación 4 · Coherencia con los recordatorios al tasador (spec §5.2.8 · A-22).** El control
de SLA incorpora una segunda familia de avisos, dirigida al **ejecutor** y no al responsable de
área: el tasador recibe un recordatorio automático cuando la coordinación supera su umbral, y otro
a las 24 h de la visita sin informe. **IF-03 no los dispara** —son de IF-02 · AT08— pero su UI no
puede contradecirlos: un tasador que acaba de recibir "te falta coordinar" y abre la app con la
solicitud fuera del chip "Por coordinar" concluye, con razón, que uno de los dos miente.
...
- Verificar que el badge de SLA de la card **no dice "En plazo"** sobre una solicitud cuyo
  recordatorio ya se emitió.
- **El umbral no se codifica en IF-03 y no se replica** (Regla T-A · A-22 · D-17): llega sin
  ratificar y su lugar es el dato de IF-02. Esta verificación comprueba **coherencia**, no
  aritmética; si el umbral cambia, IF-03 no se toca.
...
- [ ] El umbral del recordatorio **no aparece** en el código de IF-03, en ninguna forma: llega sin
      ratificar y su domicilio es el dato de IF-02.
...
*Última actualización: 21-ago-2026 · **v1.1 del plan** (segunda tanda de audios del cliente y
...
listados sin editar en §16 · riesgos conocidos en §17) · Base: Especificación v1.9.13 §2 ·
Patrón: `docs/_md/plan-ejecucion-if02-v1_9.md` v1.13*
