...
- **Valor**: 8 horas hábiles desde la asignación · **Fuente**: audios `p5` y `p7` · **Estado**: pendiente de ratificación por el product owner (**A-22**).
- La misma tanda de audios menciona 4, 6 y 24 horas en distintos momentos. Las 4 y 6 horas son los umbrales de la etapa 2 ya fijados en la matriz; las 24 horas son el tope al cliente que se define abajo. El recordatorio es un tercer número y por eso necesita ratificación propia.
...
| **2. Coordinación (llamada)** | El tasador recibe la solicitud en su bandeja. Sistema registra timestamp de asignación. Alerta si supera 4 h sin gestión, y recordatorio automático al tasador por correo y WhatsApp al cumplirse el umbral de la sección 3.3. |
...
- Recordatorio automático al tasador —correo y WhatsApp— cuando la coordinación supera el umbral de la sección 3.3, y cuando el informe lleva 24 horas pendiente desde la visita. Estos recordatorios se dirigen al ejecutor para que actúe; la notificación de rojo se dirige al responsable de área para que escale. Son mecanismos distintos y no se sustituyen.
...
El catálogo de valores admisibles de cada campo vive en la misma plantilla, en columnas ocultas. Dónde se aloja todo esto dentro del sistema es una decisión de arquitectura de datos pendiente (**A-27**); que el tasador reciba la hoja pre-llenada es un compromiso de servicio y no depende de esa decisión.
