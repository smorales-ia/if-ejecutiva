...
> **Versión**: 1.7 — alineada a Blueprint v2.10 · Capa de Datos v2.6.5 · Especificación v1.9.9 · schema-airtable.md · diseno.md · 06-jul-2026 · Fase Adjuntos 1 (D-11 a D-14, 10-jul-2026) · Re-alineamiento a Especificación v1.8.2 (17-jul-2026) · Maqueta v1.9 integrada a `main` + REGLAS A/B/C (22-jul-2026, ver §2bis).
>
...
| P4 ✅ 22-jul-2026 | Formulario 4 secciones | REGLA B ya existía casi completa; P4 agregó scroll al resumen de errores (§5.1.5) y wording canónico "N campos con problema", y restauró mensajes §6 (email/dirección literales, RUT adaptado a comprador). Verificado gating "En trámite"/"Financiero". Deuda: RN-45 por superficie, submit real (→ P9). Alcance: solo validación. |
...
| SLA en riesgo | `OR(FIND("VENCIDO", {semaforo_sla}), FIND("EN RIESGO", {semaforo_sla}))` ⚠ ver nota |
...
> ⚠ **Literales de `semaforo_sla`.** La fórmula viva **nunca emitió** `"rojo"` / `"ambar"` /
> `"verde"`: emite `VENCIDO`, `EN RIESGO`, `OK` y `Entregado`, con los emoji mangleados a `?`.
> Un filtro por igualdad contra los colores devuelve **cero filas para toda la tabla**. El
> código ya busca por subcadena desde la Tanda D-01, así que funciona con la fórmula actual y
> seguirá funcionando con la corregida. Detalle y fórmula de reemplazo en
...
> v1.9.9 §5.2.2 lo fija en la recepción del correo. Divergencia registrada como **CI-005**.
