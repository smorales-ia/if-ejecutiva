...
if (!Array.isArray(atributosObtenidos)) {
  console.error(`AT03-Ext: atributos_obtenidos de ${adjuntoId} no es un array — salir.`)
  await tAdjuntos.updateRecordAsync(adjuntoId, { estado_extraccion: 'error' })
  return
}
...
function coercionar(field, valor) {
  switch (field.type) {
    ...
    case 'singleSelect':
      return { ok: true, value: String(valor) } // la opción debe existir; si no, updateRecordAsync lanza y se captura.
...
async function escribirDestino(destTable, tablaDestinoNombre, destRow, campoDestino, item) {
  ...
  try {
    ...
    propError++
    propLog.push(`${item.codigo_atributo}: error al escribir ${tablaDestinoNombre}.${campoDestino}: ${e.message} — skip`)
    console.warn(`AT03-Ext: error al escribir ${tablaDestinoNombre}.${campoDestino}: ${e.message}`)
  }
}
...
async function resolverUnidad(campoLinkUnidad) {
  ...
  const match = q.records.find((r) => {
    ...
    return enSolicitud && String(r.getCellValue(campoClave)) === String(valorClave)
  })
  ...
  if (match) {
    ...
    unidadId = await tUnidades.createRecordAsync({
