...
### 2026-09-17 — IF-03 · alta de H1-H4 (Met_6283) como fuentes de datos (T3-T7 en main)
**Contexto:** cablear los 4 documentos nuevos del set Met_6283 —certificado_deuda_tgr (H1), consulta_antecedentes_bien_raiz (H2), informe_no_expropiacion_serviu (H3), inscripcion_dominio_cbr (H4)— en todo el flujo del Tasador, trabajando directo en `main`.
**Hallazgo 1 (esqueletos sin cablear):** las 45 filas de `D_TipoDocumentoAtributo` de los 4 tipos tenían sólo `codigo`/`codigo_atributo`/`obligatorio`; **cero** `uso_tabla_destino`. Como permiso/recepción antes: catálogo a medio cablear, no bug de UI.
**Hallazgo 2 (la "Acción propuesta" del xlsx divergía de la UI real):** el xlsx sugería rutear `n_certificado`(H3) a `TX_DocumentosLegales`, pero la UI lee `nCertificadoNoExpropiacion` desde **`TX_DatosTasacion.n_cert_no_expropiacion`** (`fldXEBNjeTRMROQmW`). Por Regla T-A/B/C (UI = fuente de verdad) se cabló al campo que la UI ya lee. Verificación imprescindible: contrastar cada `uso_campo_destino` propuesto contra `lib/tasador/lectura-datos.ts` / `lectura-informe.ts` antes de PATCHear.
**Hallazgo 3 (columnas inexistentes):** `afecto_expropiacion`, `contribucion_total`, `avaluo_afecto` **no existen** en el modelo; `deuda_contrib` existe pero la UI no lo lee. Se cableó sólo lo que existe **y** la UI lee (patrón permiso: 3/15, 3/8, 3/12, 0/10). El resto → brecha documentada.
**Decisión H4 (doble destino vendedor/comprador):** destino canónico = `TX_Solicitudes.vendedor_*` (poblado en intake); el CBR queda como origen_dato de verificación y **no se rutea** (además el resolver 1:1 de AT03-Ext no alcanza `TX_Solicitudes`). Documentado en Origen §2.1.6.
**Solución aplicada:** (T3) PATCH MCP a 9 filas (H2×3, H3×3, H4×3), `una_por_solicitud`, `uso_interfaz_tasador=TRUE`, tipo_dato text/number, ejemplos Met_6283. H1 sin cableado. (T4/T5) alta de los 3 tipos con destino en `CAMPOS_DERIVADOS` (`lib/adjuntos-doc-campos.ts` v1.3) + `FIELD_IDS_DATOS_TASACION.avaluoTotal`; CASCADE_REGISTRY 9→12; tests `adjuntos-cascade.test.ts` y `adjuntos-doc-campos.test.ts` recuadrados + cobertura nueva (842 verdes). Ojo: dos tests usaban `consulta_antecedentes_bien_raiz` como ejemplo "sin destino" → se movieron a `certificado_deuda_tgr` (H1, que sí queda sin destino). (T6) typecheck + `next build` limpios. (T7) Origen v1.6→**v1.7** (§2.1.4-2.1.7, `git mv`).
**Deuda de despliegue (sólo Sergio):** el poblado runtime depende de AT03-Ext **v3** pegado en Airtable UI y de SC-RF09/Make provisionado; el ciclo upload→extracción→AT01-AT10→borrado es prueba post-deploy, no ejecutable localmente. Todo lo controlable quedó verde localmente.
**Prevención futura:** un `uso_campo_destino` sólo es válido si (a) la columna existe en la tabla real (meta API) y (b) algún adaptador de la UI la lee; verificar ambos antes de escribir en `D_TipoDocumentoAtributo`. La "Acción propuesta" de un xlsx de análisis es hipótesis, no contrato.

### 2026-09-17 — Auditoría del motor real (xlsm MET-6283) vs AT03 v32
**Contexto:** cambio de estrategia — auditar `docs/_referencias/1951-MET 6283-…xlsm` (motor Excel que produjo el informe) para extraer las fórmulas reales y compararlas con C_Formulas v3.1/v3.2. Salida en `claude-out.txt` (sólo diagnóstico).
**Inconveniente:** el Plan v3 (Bloqueo #3) asumía que el terreno del informe se homogeneiza por comparables a ~2,23 UF/m²; con esa premisa se iba a poblar `TX_Comparables` para destrabar la valuación.
**Causa raíz:** el "2,23 UF/m²" del cuadro es un valor **derivado/cosmético** (`= UF_terreno_total / sup_terreno` = 11.218,80 / 5.024,86). El motor real (hoja `Portada`, filas 51–52) tasa el terreno con **dos filas manuales**: 1.402,35 m² × 8 UF/m² + 3.622,51 m² × 0 UF/m². No hay comparables en el cálculo del terreno.
**Solución aplicada:** extracción con openpyxl (2 pasadas formula/valor) + tracer de dependencias con regex corregido (el primero tragaba la 1ª letra de columnas de 2 letras, BI→I). Se localizaron y verificaron aritméticamente los 13 valores del oráculo + subtotales. Diagnóstico: el único cálculo estructuralmente roto en v3.2 es **F_ValorComercialUF** (computa sólo edificación, omite terreno+OCC); #7 Avalúo, #12 Ingreso, #13 Renta ya coinciden exactas. Recomendación: Ruta 3 (override `valor_final_override=20.125,86` → 11/13; +reposición/seguro → 13/13) ahora; Ruta 2 (patches P1–P3 a C_Formulas) como fix estructural.
**Prevención futura:** en un xlsm de tasación, distinguir celdas-driver (input) de celdas-display (promedios/ratios derivados) antes de asumir la mecánica; un UF/m² "promedio" en el cuadro suele ser cociente, no precio. openpyxl `data_only=True` sólo trae el último valor cacheado por Excel — si el libro no se recalculó, esos valores pueden faltar (aquí estaban presentes).

### 2026-09-18 — Prueba prod MET-6283: overrides + AT03 v32 (13/13)
**Contexto:** aplicar Ruta 3 (3 overrides) sobre sandbox VP-2026-0066 y disparar AT03 para validar los 13 valores del oráculo contra el informe.
**Inconveniente:** el plan asumía que con la regla asignada (v3.1) + overrides se obtendría 13/13. El primer disparo dio **19 filas UF/intermedias sin ningún terminal CLP** ni avaluo_fiscal_uf.
**Causa raíz:** Bloqueo #1 vivo — AT01 asigna `Regla_MetLife_Refinanciamiento_Casa` (v3.1), cuyas `formulas_resultado` son UF + intermedias, **no** los 13 terminales CLP. La regla, no el override, determina qué fórmulas corre AT03. El override sólo afecta a los valores que la regla efectivamente evalúa.
**Solución aplicada:** Opción A del Bloqueo #1 — PATCH `regla_aplicada = REGLA_REFI_CASA_V32` (recYEf9XepX4SmLnH), verificar que AT01 no la revirtió, y re-disparar (estado asignada→visitada). `AT03_Calculos_DAG` hace CLEANUP de TX_Calculos previos (línea 680) → reemplazó limpio 19→13 filas. Resultado **13/13 dentro de ±1%** (deltas <0,00003%, por overrides a 2 decimales vs xlsm 20125,8624/8907,0624; uf_dia_visita exacto). Propuesta P1–P6 en `docs/_analisis/PROPUESTA_MOTOR_v32_fix_MET6283.md`.
**Observaciones:** AT03 no escribió A_Eventos y el estado quedó en `visitada` (AT04 no corrió/inactivo en sandbox). `TX_ObrasComplementarias` (tblQ1fXM06bzSQ84w) ya existe como hogar de OCC; el DAG ya la lee — el gap es de poblado, no de esquema. No existe tabla de tramos de terreno (el split 1402,35@8 + resto@0 no tiene dónde vivir).
**Prevención futura:** para reproducir un oráculo con AT03, verificar **primero** que `regla_aplicada.formulas_resultado` contenga las variables_output esperadas; el override no suple fórmulas ausentes. Al re-disparar AT03, confiar en su CLEANUP para evitar duplicados, y reconfirmar la regla tras cualquier ciclo de estado (por si AT01 reasigna).

### 2026-09-18 — Fase B P##-MOTOR: bloqueada por límite del motor de C_Formulas
**Contexto:** aplicar patches P1–P3 para que el comercial sume terreno+OCC desde `TX_ItemsCuadroValoracion` (diseño v3, Fase A) y regresar contra la sandbox VP-2026-0066.
**Inconveniente:** el diseño v3 quiere `F_ValorComercialUF = F_ValorEdificacion + F_ValorTerreno + F_ValorOCC` con terreno/OCC por SUMIF sobre `TX_ItemsCuadroValoracion`. No es aplicable tocando solo `C_Formulas`.
**Causa raíz:** el evaluador de `AT03_Calculos_DAG.js` (`safeEval`) solo ve variables escalares del `SCOPE` + outputs de fórmulas previas; **no lee tablas hijas**. La agregación de hijos se hace en JS antes de armar el SCOPE (`sumObrasComplementarias` para OCC vía `TX_ObrasComplementarias`; `M_Comunas` para terreno). El DAG **no** abre `TX_ItemsCuadroValoracion` (no está en la lista `getTable`, líneas 50-65) y el SCOPE no tiene `valor_terreno_uf`/`valor_occ_uf` derivados del cuadro. `F_ValorTerreno` (recHNKyfGDBV4Hxu3, v3.1) hoy = `sup_terreno_m2 * uf_m2_terreno_comuna` → 5.024,86×17 = 85.422 UF (oráculo 11.218,80); comercial proyectado +369 % → FAIL.
**Solución aplicada:** ninguna escritura. DETENER y documentar (`docs/_analisis/FASE_B_APLICACION_PATCHES.md`). Definida "Fase B-0" de desbloqueo: extender el DAG para leer `TX_ItemsCuadroValoracion` y sumar `valor_total_uf` (fld1F3u5J5NlnJUjY) por `tipo_bien` (fld5HVdWpMY0jWqkx), inyectar `valor_*_items_uf` al SCOPE, **re-desplegar el script en la Automation AT03** (pegado manual — el MCP no edita scripts de Automation), y recién ahí parchar `C_Formulas` (v3.3), resolver Bloqueo #2 (calidad→BUENA) y regresar.
**Prevención futura:** antes de comprometer un diseño de motor que dependa de agregar tablas hijas, verificar que el ejecutor (DAG) realmente lea esa tabla y exponga el agregado en el SCOPE; `C_Formulas` no puede hacer SUMIF por sí solo. Los overrides de la sandbox se dejaron puestos: el motor sigue dando 13/13 vía override; NO retirarlos hasta completar Fase B-0.

### 2026-09-21 — Cierre B-0b MET-6283: MCP Airtable caído → fallback REST; 13/13 real
**Contexto:** TANDA B-0b, tras publicar v11.1.1 en AT03. Verificar el borrador del DAG, disparar `asignada→visitada`, validar los 13 TX_Calculos (BLOQUE 2), auditor ciego (BLOQUE 3) y rollback condicional (BLOQUE 4).
**Inconveniente:** el plan asumía stagear el DAG por `mcp__Airtable__update_automation` y verificar con `get_automation`. Ninguna de esas tools existía en la sesión; el MCP `airtable` conectado solo exponía el flujo OAuth y, tras autenticar, sus scopes (`data.records`/`schema.bases`/`data.recordComments`) **no incluyen automations**. Además el propio OAuth falló: el authorize devolvió `invalid_request — "code_challenge" must be a valid url-encoding of the sha256 of the code_verifier` (bug PKCE del lado del servidor MCP), y un primer callback pegado era de otro flujo (`state mismatch`).
**Causa raíz:** el servidor MCP con capacidades de automation (mayúscula `Airtable`, usado en sesiones previas) **no estaba conectado** en esta sesión; el oficial `mcp.airtable.com` no cubre automations ni lee logs de ejecución. El staging del DAG y el clic Publish son inevitablemente manuales de Sergio.
**Solución aplicada:** pivote a REST con `AIRTABLE_TOKEN` del `.env.local` (fallback RO-30): `curl` para read-back del sandbox, PATCH `estado='visitada'` sobre `recNiwM4s1ibr3sbO`, y lecturas de `TX_Calculos` (`tblFz37KSvn5pLKDR`, filtrando `solicitud_codigo`). El singleSelect `estado` por REST acepta el string pelado (a diferencia de Airtable Scripting, que exige `{name:...}` — origen del fix v11.1.1). Resultado: 13/13 dentro de ±1% (Δ≈0), `version_motor=AT03_v11.1.1_v32b0`, `estado=calculada` (fix confirmado en runtime), 0 contaminación. Auditor ciego (agente independiente) → CONSISTENTE 13/13. BLOQUE 4 no gatillado.
**Observaciones:** `.env.local` línea 23 tiene una entrada que no es asignación de variable (`completar: command not found` al hacer `source`) — inofensivo pero conviene comentarla con `#`. Límites de capacidad declarados: los asertos sobre log de Automation (I-01..I-07) y los screenshots UI (S-05) no son ejecutables por mí; los primeros se cubren indirectamente por estado final, los segundos los toma Sergio. El `git push` falló (`could not read Username for github.com`): la CLI no tiene credenciales, el push lo hace Sergio por GitHub Desktop.
**Prevención futura:** para tareas que toquen Airtable Automations (leer/editar scripts, publicar, leer logs), confirmar al inicio que el MCP con scope de automations está conectado; el `mcp.airtable.com` oficial NO sirve para eso. Para verificación de datos, el fallback REST con `AIRTABLE_TOKEN` es suficiente y no depende del MCP. El staging de scripts de 67k y el Publish son siempre manuales.

### 2026-09-21 — Handoff RF09-REACTIVACION automatizado (Make REST + MCP Airtable)
**Contexto:** BLOQUE 2 de FASE 2. Los 8 pasos del handoff estaban escritos como Sergio-manual (checklist en `claude-out.txt`); esta sesión los ejecutó por API para reducir el trabajo manual a cero. Resultado: 8/8 OK, cero residuales.
**Inconveniente 1 — MCP de automations SÍ disponible esta vez.** A diferencia del cierre B-0b (misma fecha), en esta sesión las tools `mcp__claude_ai_Airtable__{list,get,update}_automation` estaban conectadas y funcionaron (list/get). El servidor con scope de automations es `claude_ai_Airtable` (namespace `mcp__claude_ai_Airtable__*`), no el `mcp.airtable.com` OAuth. **Prevención:** verificar el namespace exacto de las tools MCP al inicio; la disponibilidad varía entre sesiones.
**Inconveniente 2 — el PASO 5 no era residual manual.** El plan asumía que los "Secrets" de Airtable Automations no son gestionables por API. Pero `AT-RF09-Trigger` (wflIEucD1MxxcNXH8) **no usa la feature Secrets**: pasa `MAKE_WEBHOOK_URL_RF09` y `MAKE_RF09_HMAC_SECRET` como input variables normales (`input.config()`), visibles y editables vía `get_automation`/`update_automation`. La URL almacenada ya coincidía exacto con el hook 3393157. **Prevención:** antes de declarar un valor "no accesible por API", leer cómo lo consume el script — input variables ≠ Secrets.
**Inconveniente 3 — teamId vs organizationId en Make REST.** `GET /scenarios?organizationId=1594725` → 403; `?teamId=1594725` → 200. En esta org el mismo número 1594725 funciona como teamId, no como organizationId. `POST /connections/{id}/verify` → 404 (no existe en esta cuenta); la validez se infiere de `GET /connections/{id}` (expire=null, scoped) + `scenario.isinvalid=false`.
**Inconveniente 4 — PATCH de blueprint y contrato de timeout.** El update es `PATCH /api/v2/scenarios/{id}` con body `{"blueprint": "<json string stringificado>"}` (el string puede ir compacto). El módulo 10 (`http:ActionSendData` v3) tiene DOS claves `timeout`: la efectiva es `parameters.timeout` (="180"); `mapper.timeout` ("") es un leftover inerte — no confundir al auditar. El blueprint Make **no verifica** la firma HMAC (sin `x-vp-signature`/`hmac`/`sha256`), así que el `MAKE_RF09_HMAC_SECRET` del script es cosmético. `AT02_Asignar_Tasador` figura como `deploymentStatus=undeployed` = OFF (confirmación por API, cerrando el hueco S-05 del cierre anterior).
**Solución aplicada:** backup del blueprint previo (`docs/_artefactos/make/backups/SC-RF09_pre-v22_<ts>.json`) antes de tocar; dos PATCH (v2.2 con placeholder → verificación estructural; luego key inyectada en `/tmp` y borrada con `shred`). Cero writes en tablas de datos. Cierre completo en `claude-out.txt`. Claves y token nunca impresos.

### 2026-09-22 — Desbloqueo FASE 2 RF-09 con key rotada + reintento de extracción
**Contexto:** Sergio proveyó un `ANTHROPIC_API_KEY` nuevo (108 chars, `sk-ant-api03-m…`) para desbloquear la extracción RF-09 de VP-2026-0067 (sandbox MET-6283). Procedimiento de 7 pasos: validar key → re-inyectar en módulo 10 (scenario Make 6554321) → reactivar → actualizar `.env.local` → reintentar los 8 adjuntos → verificar → detectar gaps FASE 3.
**Inconveniente 1 — auditar el `timeout` en el lugar equivocado.** Al leer el blueprint, `mapper.timeout` del módulo 10 (`http:ActionSendData` v3) sale vacío (`''`) y disparó una falsa alarma de "timeout perdido". El valor efectivo vive en `parameters.timeout` (="180"); `mapper.timeout` es un leftover inerte. **Prevención:** en módulos HTTP v3 de Make, verificar `parameters.timeout`, no `mapper.timeout` (ya anotado el 21-09 pero volvió a morder).
**Inconveniente 2 — poller REST leyó MISSING en los 8 estados.** Un script bash de polling que hacía `GET /v0/{base}/{table}/{recId}` y buscaba el campo por FIELD_ID (`fld54epvDJ7YdJIYD`) devolvió `MISSING` para los 8: la REST API de Airtable devuelve los campos **por NOMBRE** por defecto. **Causa raíz:** falta `?returnFieldsByFieldId=true` en la query. **Solución:** verifiqué los estados finales por MCP (que sí devuelve `cellValuesByFieldId`). **Prevención:** al pollear por FIELD_ID vía REST, añadir `returnFieldsByFieldId=true`, o leer por nombre de campo.
**Inconveniente 3 — los 4 PDF "no-op" no van a 'skipped' sino a 'listo'.** El plan (PASO 6.2) asumía que certificado_deuda_tgr, certificado_recepcion_final, consulta_antecedentes_bien_raiz e informe_no_expropiacion_serviu llegarían a `skipped`. En runtime el scenario RF-09 SÍ los manda a Claude y quedan `listo` con JSON extraído. No es fallo — pero consume llamadas del presupuesto (7 de 8: 3 POBLA + 4 no-op). **Prevención:** el "no-op" era de intención de negocio, no de comportamiento del scenario; contar 7 llamadas Claude por corrida de estos 8 docs, no 3.
**Inconveniente 4 — el .docx queda colgado en 'extrayendo', no en 'error'.** `inscripcion_dominio_cbr.docx` no llegó a estado terminal tras ~15 min (probable 400 `media_type` en módulo 10; el path async no escribió `error`). Esperado que fallara, pero el estado colgado ensucia el read-back. **Prevención:** para docs no-imagen, tratar `extrayendo` persistente como fallo silencioso equivalente a `error`; no es POBLA, no bloquea FASE 3.
**Hallazgo FASE 3 (gaps):** el motor DAG necesita inputs que NO derivan de los 8 docs y sólo existían en VP-2026-0066: (A) los 6 ítems de `TX_ItemsCuadroValoracion` (cuadro de valoración: Edificacion/2×Terreno/Piscina/2×OO.CC.), 0 en 0067; (B) 10 campos de visita en `TX_DatosTasacion` (estado_conservacion, material_predominante, anio_construccion, sup_construccion_m2, sup_terreno_m2, arriendo_bruto_mensual_clp, gasto_anual_clp, tasa_cap_rate, uf_dia_visita, velocidad_venta_estimada). `TX_Unidades` está vacío en ambos → no es gap. Se DETUVO antes de `POST /calcular` esperando decisión de Sergio (mirror desde 0066 / sembrado manual / omitir). Sin commits, sin push.

### 2026-09-22 — Auditoría de inputs del motor vs. RF-09 y UI del tasador (huecos de producto)
**Contexto:** antes de decidir el mirror-desde-0066 para FASE 3, Sergio pidió auditar (solo lectura) qué inputs del motor DAG v11.1.1 NO vienen de los 8 docs RF-09 y cuáles tienen UI de captura. 3 agentes en paralelo (motor / fuentes / UI); entregable `docs/_analisis/AUDITORIA_inputs_motor_MET6283.md`.
**Hallazgo 1 — ninguno de los 16 gaps viene de RF-09.** RF-09 puebla TX_Comparables, campos SII de TX_DatosTasacion y TX_DocumentosLegales, pero `D_TipoDocumentoAtributo` no tiene NINGÚN atributo con `uso_tabla_destino=TX_ItemsCuadroValoracion`: la tabla del cuadro de valoración no tiene escritor automático en todo el sistema. Los campos de visita de TX_DatosTasacion tampoco los escribe RF-09 (algunos como `tipo_material`/`anio_construccion`/`sup_terreno_m2` sí se extraen pero se enrutan a TX_Unidades, no a DatosTasacion).
**Hallazgo 2 — huecos de UI reales (no derivables ni por RF-09 ni por tasador).** La grilla del cuadro (sección C, `seccion-valoracion.tsx`) captura sup_m2/descripción/subtipo pero NO `uf_m2_unitario` ni `factor`; como `valor_total_uf`/`valor_seguro_base` son fórmulas Airtable = f(sup_m2, uf_m2_unitario, factor), sin esos 2 el cuadro suma 0 aunque existan filas. Además `tasa_cap_rate` base y `uf_dia_visita` no tienen input. → HP-1..HP-4 en el doc. 8 de los 16 gaps SÍ tienen UI (secciones B y H).
**Hallazgo 3 — AT03-Ext sin blueprint en el repo.** El scenario SC-RF09 sólo escribe a TX_Adjuntos y loguea "Enrutamiento por cardinalidad delegado a AT03-Ext"; la automatización que enruta a las tablas normalizadas corre en prod (lo prueban los 7 comparables y el DocumentoLegal de 0067) pero no está en `docs/_artefactos/make/`. Deuda de artefacto.
**Conclusión:** mirror-desde-0066 es válido para cerrar el SANDBOX (única vía para el cuadro, que no tiene UI para sus campos económicos), pero NO resuelve el producto: HP-1/HP-2 son huecos reales para cualquier tasación por cuadro no-MetLife. Se dejó como issues de tanda futura. Auditoría solo-lectura; único archivo escrito el de análisis. Sin commits.

### 2026-09-22 — FASE 2 T-MC-P0 · rutas WSL y hallazgos de ejecución
**Contexto:** ejecución del cierre de inputs del motor (H1/H2/H3/overrides/RF-09) en rama `feat/T-MC-P0`.
**Inconveniente 1 — los cierres a `claude-out.txt` no llegaban al disco.** El tool `Write` con la ruta en formato Windows `C:\Users\Sergio\Documents\claude-out.txt` crea un archivo **literal** con backslashes en el nombre dentro del cwd del repo (WSL trata `\` como carácter válido de nombre, no como separador). Los cierres FASE 1 quedaron como archivo fantasma untracked en el repo; nunca llegaron a Documents. Peor: un `ls` del mount mostraba un `claude-out.txt` viejo real, y "confirmé presencia" sin verificar frescura. **Causa raíz:** ruta Windows en entorno POSIX + verificación por presencia en vez de contenido/fecha. **Solución:** escribir SIEMPRE a `/mnt/c/Users/Sergio/Documents/claude-out.txt`; borré el fantasma; verifiqué con `head -1` + fecha. **Prevención:** en WSL, nunca pasar rutas `C:\...` a herramientas de archivo — usar `/mnt/c/...`. Si aparece un archivo con backslashes en `git status`, es este bug.
**Inconveniente 2 — `fecha_visita` no es un campo real.** El guard H3 debía hacer lookup a `H_PreciosUF` por `fecha_visita`, pero ese campo NO existe ni en `TX_Solicitudes` ni en `TX_DatosTasacion` (422 al pedirlo por nombre). VP-2026-0066 tampoco tiene fecha de visita poblada (createdTime 2026-09-17). Se implementó el guard en su forma segura (fallar ruidoso si `uf_dia_visita` viene vacío, sin default 38500) y se dejó el lookup-por-fecha pendiente de identificar el campo real. **Prevención:** verificar existencia del campo por schema antes de codificar un lookup que lo asume.
**Inconveniente 3 — `factor_df_calc` (SCOPE del DAG) ≠ `factor_aplicado` (fórmula del ítem).** El DAG ya deriva `coefEstado` desde `estado_conservacion` y lo expone como `factor_df_calc` (:1052), pero la fórmula `valor_uf` del cuadro multiplica por el `factor_aplicado` de cada fila de `TX_ItemsCuadroValoracion`, que es OTRO valor. Por eso H2 necesita override por ítem en la UI (el 0.96 del xlsm ≠ 0.95 derivable de "Bueno"): el default del motor no reproduce MET-6283. **Prevención:** no confundir el factor de SCOPE del DAG con el factor por ítem del cuadro; son dos cosas.
**Inconveniente 4 — AT03-Ext v3 ya tiene guards.** Antes de "agregar el guard no-pisar-tipeado", se verificó que AT03-Ext v3 ya escribe `origen_dato='extraido_rf09'` y ya sólo escribe destinos vacíos ("solo si vacío", :322/:376). Editar ese script vivo a ciegas habría duplicado/roto lógica. Se dejó el repunte de `D_TipoDocumentoAtributo` (3 filas: `recHzaRLGLOueQWKY` anio_construccion, `recYwrGMxW0PGqbxe` sup_terreno_m2, `recycv6hnoK9krfxg` sup_m2) documentado y DIFERIDO al paste guardado, para no abrir la ventana de contaminación que el guard busca cerrar. **Prevención:** leer la función de propagación completa de un script de producción antes de editarlo; aplicar cambios de enrutamiento junto con (no antes de) el guard que los protege.

### 2026-09-23 — Auditoría del "DRIFT MASIVO" pre-paste T-MC-P0 (falso drift CRLF)
**Contexto:** STOP crítico pre-paste del paquete `docs/_paste/T-MC-P0_paste-instructions.md`. La sesión del 22-09 abortó al detectar que `diff repo vs live` del DAG AT03_Calculos reportaba `1,1365c1,1400` — TODO el archivo cambiado + cabecera nueva — en vez de las 3 ediciones esperadas (handle H_PreciosUF, fetch fecha_visita, guard H3). Se temió pisar cambios live no versionados.
**Inconveniente 1 — el "drift masivo" era un artefacto de line endings, no drift real.** La captura live (`AT03_Calculos_DAG_pre-T-MC-P0.js`) se guardó con **CRLF** (1361 CR); el archivo del repo es **LF** (0 CR). `diff` sin normalizar marca las 1365 líneas como distintas porque cada línea difiere en el `\r` final. **Causa raíz:** comparar un archivo capturado por MCP/editor con CRLF contra un archivo del repo con LF, usando `diff` a secas. **Solución:** `diff --strip-trailing-cr live repo` colapsa a 5 hunks; uno es la cabecera de backup de 4 líneas y los otros 4 son EXACTAMENTE las 3 ediciones T-MC-P0. Cero drift real: repo = live + los 3 parches, limpiamente aplicados. Se materializó una copia LF del live (`AT03_Calculos_DAG_LIVE_20260923.js`) para que futuros diffs no repitan la trampa. **Prevención:** antes de declarar drift entre live y repo, verificar line endings (`file`, `grep -c $'\r'`) y diffear con `--strip-trailing-cr`. Un `Nc M` que abarca el archivo entero casi siempre es CRLF, no reescritura.
**Inconveniente 2 — MCP `update_automation` sólo escribe el DRAFT, no publica.** Confirmado el 22-09: `update_automation` modifica el borrador de la Automation pero el deploy requiere pulsar "Update" en la UI de Airtable. Cualquier flujo que asuma "MCP pastea y queda desplegado" está roto por diseño. Además, el server oficial `mcp.airtable.com` (scopes `data.records`/`schema.bases`/`recordComments`) **no lee automations**: la captura live previa vino de otro MCP. Por eso esta sesión no pudo re-extraer AT03-Ext (su live no se salvó a disco antes del apagón). **Prevención:** tratar el paste de automations como operación 100% manual en la UI; guardar SIEMPRE la captura live a disco (LF) en el mismo paso en que se genera, no al final.
**Inconveniente 3 — AT03-Ext sin baseline live comparable.** El único backup en disco de AT03-Ext es del 11-09 (12 días viejo); diffearlo contra el repo mezcla la edición T-MC-P0 (guard `origen_dato=tipeado`) con cambios Sep11→Sep22 no relacionados (9 hunks). El guard T-MC-P0 SÍ está presente en el repo (líneas 333-344), pero aislar que no haya drift adicional exige una captura live fresca. **Prevención:** no dejar automations editadas sin snapshot live fechado; el backup por fecha de calendario no sustituye la captura del deployed.

### 2026-09-23 — Validación post-deploy T-MC-P0 · warning A_Eventos y defecto de auditoría del DAG
**Contexto:** tras el deploy del DAG (H3) y AT03-Ext por Sergio, corrida OK sobre VP-2026-0066 (13/13 TX_Calculos, FIN OK 6141 ms). Se pidió ejecutar pasos 8/10 (comparar 13 terminales vs oráculo MET-6283 ±1%) y diagnosticar el warning `ERROR A_Eventos[AT03]: Request processing is disabled due to earlier failed request`.
**Resultado validación:** 13/13 terminales al 0,00% vs oráculo b-0b. H3 confirmado: `valor_liquidacion_clp = 662.403.580,87 = 16.603,84 UF × 39.894,61` (UF del 13-04-2026 por lookup a `H_PreciosUF`); no abortó y no usó el default 38500 (habría dado 639,2 M). T-MC-P0 VALIDADO.
**Inconveniente — el warning A_Eventos es un mensaje SECUNDARIO que oculta el error real.** El texto "Request processing is disabled due to earlier failed request" lo emite el 2º intento (`ultraMin`, línea 217 del DAG) porque el 1er intento (`minimalMap`, línea 207) ya falló en la misma cadena; el error real del 1er intento se traga en el `catch e1` sin loguear. **Causa raíz:** `A_Eventos` NO tiene campo `mensaje` (su schema real: `tipo_evento`/`descripcion`/`detalle_json`/`severidad`/…); el map que arma `logEventoCompleto` es rechazado al crear el record. Empírico: **0 filas A_Eventos creadas en la corrida** (ni el evento inicial de línea 699 ni el final de 1349). **Es benigno al cálculo** (las 13 escrituras y la transición estado→calculada ocurren en awaits top-level posteriores y salen OK; el helper está envuelto para no ser fatal, `return null`), pero es un **defecto de auditoría preexistente** (no introducido por T-MC-P0, que sólo tocó H3/H_PreciosUF): el motor no deja rastro en A_Eventos. **Prevención:** (1) cuando Airtable scripting diga "disabled due to earlier failed request", buscar el PRIMER await que falló, no el que emite el mensaje; loguear el `catch e1`, no sólo el `e2`. (2) `logEventoCompleto` debe mapear a los campos reales de A_Eventos (`descripcion`/`detalle_json`), no a un `mensaje` inexistente. Issue abierto aparte; no bloquea T-MC-P0.

### 2026-09-23 — Cierre T-AUDIT-CLOSE tras interrupción de forks (límite Pro) · auditor ciego + C3 + M4
**Contexto:** retoma de la tanda T-AUDIT-CLOSE-20260923 en sesión Max; los forks FORK1/FORK2 de FASE 2 habían muerto por límite Pro dejando el CIERRE escrito pero incompleto.
**Inconveniente 1 — el doc de cierre afirmaba cosas que el disco no respaldaba.** `CIERRE_...md` referenciaba un `auditor.md` inexistente, listaba "AT08 con `clave_notif`" cuando el mecanismo real es `clave_natural` con prefijo `AT08:`, y omitía en silencio que C3 (gating H4) y M4 (cron UF) no se habían ejecutado. **Causa raíz:** el fork escribió el CIERRE antes de completar los pasos y murió en medio. **Solución aplicada:** verificación disco-vs-doc (grep de guards, wc de artefactos, re-run de tests) antes de confiar en el CIERRE; auditor ciego (agente general-purpose sin contexto heredado) que produjo `auditor.md` con veredicto CIERRA CON RESERVAS; las 3 reservas se cerraron en la misma sesión y el delta quedó en CIERRE §8. **Prevención futura:** un doc de cierre escrito por un agente interrumpido es una AFIRMACIÓN, no evidencia — contrastar contra disco y re-ejecutar los gates antes de retomarlo; el auditor ciego post-tanda detecta exactamente esta clase de divergencia.
**Inconveniente 2 — C3 estaba bloqueado por una pregunta de OK-Gate sin responder (Q2: qué `tipo_informe` exige rentabilidad).** **Causa raíz:** el gating necesitaba una regla de negocio que ni spec ni schema definen, y hardcodearla violaría "la UI nunca decide". **Solución aplicada:** Opción 1 consensuada con Sergio — checkbox `requiere_rentabilidad` en `M_TiposInforme`; la UI proyecta y obedece (`lib/tasador/rentabilidad.ts` + `lectura-tasacion.ts` + `tasacion-form.tsx`), con `null` = fail-safe (visible/opcional, comportamiento pre-C3). La lectura del flag NO reusa `fetchCatalogos()` para no tocar `lib/catalogos.ts` (territorio IF-02, §8 NO-TOCAR). **Prevención futura:** cuando una regla de negocio no tiene dueño en datos, la salida no es elegir valores en código sino crear el dato maestro y degradar a `null` explícito, nunca a mapa vacío (vacío habría ocultado la sección para todos ante un fallo de lectura).
**Inconveniente 3 — un checkbox de Airtable no distingue `false` de "vacío".** Desmarcado llega ausente por API; con la semántica "desmarcado = ocultar", desplegar el código C3 con el catálogo sin poblar ocultaría la sección H para todos los tipos. **Solución aplicada:** implicación documentada en `C3-H4-decision.md` y en los pasos manuales del CIERRE: poblar los flags ANTES de mergear/desplegar C3; mientras el campo no exista rige el fail-safe. **Prevención futura:** al introducir un flag checkbox que apaga UI, planificar el orden campo→poblado→deploy como parte del diseño, no como nota al pie.

### 2026-09-23 — Despliegue manual T-AUDIT-CLOSE · límites del MCP Airtable con automations
**Contexto:** ejecución de los pasos manuales del CIERRE (campo H4, cron UF, AT03 guards, AT08 dry-run) vía MCP Airtable.
**Inconveniente 1 — los nodos `customScript` son read-only para la API/MCP.** `create_automation` y `update_automation` rechazan cualquier nodo de script (`readOnlyNodeType`), incluso preservando el key del nodo persistido. **Causa raíz:** restricción de la API de Airtable, no del MCP. **Solución aplicada:** patrón "shell + paste": el MCP crea la automation con nombre, descripción-instructivo y trigger cron ya configurados más un nodo placeholder (`findRecords` inocuo; `noOp` también es read-only), y el paste del script + Test + activar quedan como único paso UI (CRON_UF_Diaria `wflQ9NC7dHcuY6Hh0`, AT08 `wfl15j5QyoL0ttnvw`). Para una automation existente (AT03), el update entero es UI-only. **Prevención futura:** no planificar "reimport vía MCP" para scripts de automation; el MCP sirve para leer (`get_automation` sí devuelve el script) y para armar el esqueleto, nunca para pegar código.
**Inconveniente 2 — la URL OAuth del MCP se corrompe al copiarla desde la terminal.** El primer intento falló con `invalid_request` porque el wrap de línea de la terminal insertó caracteres en `state`/`code_challenge` al copiar. **Solución aplicada:** abrir la URL directamente en el navegador de Windows desde WSL con `powershell.exe -NoProfile -Command "Start-Process '<url>'"` (la URL entre comillas simples; `cmd.exe /c start` NO sirve — parte la URL en cada `&`). **Prevención futura:** nunca pedir al usuario copiar URLs largas desde la terminal; lanzarlas con Start-Process.
**Inconveniente 3 — verificar antes de crear: el campo H4 ya existía.** `create_field` devolvió `DUPLICATE_OR_EMPTY_FIELD_NAME`: `requiere_rentabilidad` (`fldv9alrfd7LlCl5K`) era parte del diseño original de M_TiposInforme, con Refinanciamiento ya marcado — exactamente la política que C3 necesitaba. **Prevención futura:** ante un plan que diga "crear campo/tabla", listar primero el schema real por nombres (`list_tables_for_base`, que sí trae `name`; `get_table_schema` no los incluye): el dato maestro puede ya existir y traer la política puesta.

### 2026-09-23 — PREP T-AT08-CLOSE · destinatarios + limpieza de cartera
**Contexto:** FASE 1+2 de T-AT08-CLOSE-20260923 — crear las 3 filas `sla_alerta_roja_*` en C_NotificacionesConfig y dejar la cartera activa en exactamente {VP-2026-0066, VP-2026-0062}.
**Inconveniente 1 — el envío real (SC13) no tiene escenario Make que lo respalde.** El script AT08 escribe TX_Notificaciones asumiendo que "Make la observa y envía (SC13)", pero en Z_EscenariosMake la única fila SC13 es `SC13_EntregaCliente`, estado Pendiente, sin scenario ID, y con otro propósito (entrega final al cliente). **Causa raíz:** el watcher de TX_Notificaciones nunca se provisionó; el nombre SC13 quedó sobrecargado con dos significados. **Solución aplicada:** documentado como dependencia abierta en `PREP_T-AT08-CLOSE-20260923.md` y en los pasos UI: si en el paso 3 no llegan correos, las 2 filas `estado_envio=Pendiente` en TX_Notificaciones ya validan AT08; lo que falta es el escenario Make, no el script. **Prevención futura:** antes de prometer "llega el correo" en un plan de prueba, verificar en Z_EscenariosMake/Make que el eslabón de envío exista y esté activo — el MCP no alcanza Make (RO-30), así que esa verificación siempre es paso UI de Sergio.
**Inconveniente 2 — el ruteo de AT08 va por etapa vigente (`sla_eN_inicio_ts` sin fin), no por `estado`.** La intuición "0066 está asignada ⇒ rutea al tasador (e2)" era falsa: 0066 tiene e4 abierta (→ control_seguimiento) y 0062 tiene e5 abierta (→ tasador). **Causa raíz:** `etapaVigente()` recorre los pares inicio/fin y el estado del select no participa; además `get_table_schema` del MCP no devuelve nombres de campo, así que el mapping FIELD_ID→nombre hubo que sacarlo de `lib/sla-etapas.ts` (fuente ya versionada). **Solución aplicada:** predicción de ruteo verificada campo a campo antes de crear las filas; quedó en `snapshot.md` §0.5. **Prevención futura:** para predecir a qué área alerta AT08, mirar los `sla_eN_*_ts` del registro (o `sla_etapa_actual`), nunca el `estado`; y para mapear FIELD_IDs sin nombre, `lib/sla-etapas.ts` es la referencia más rápida.

### 2026-09-24 — T-MAKE-SLA-ENVIO · el "despachador" de TX_Notificaciones no existía
**Contexto:** conectar Make al evento `sla_alerta_roja` para que las 2 filas Pendiente de AT08 se enviaran por correo.
**Inconveniente:** la premisa de la tanda ("existe un escenario que despacha TX_Notificaciones; falta la rama del evento nuevo") era falsa. Barrido por API de los 12 escenarios del team 1594725 (único team de la org 7487039), blueprint por blueprint: ninguno observa la tabla. El único que la referencia con escrituras reales es SC05 v1.0 (6780103), pero es un webhook del flujo de asignación, está inactivo y **nunca se ejecutó** (logs sin ejecuciones, solo start/stop); además escribe las filas ya en `Enviado` post-envío. Las filas históricas `Enviado` salieron de escenarios ya eliminados o pruebas.
**Causa raíz:** TX_Notificaciones se usó con dos semánticas mezcladas — bitácora post-envío (SC05, pipeline PDF legacy) y cola de despacho (AT08) — sin que nadie provisionara jamás el consumidor de la cola. El aviso ya estaba en `snapshot.md` de T-AT08-CLOSE ("SC13 watcher: SIN EVIDENCIA de existir").
**Solución aplicada:** con aprobación explícita de Sergio en sesión, se creó vía API `SC-SLA-Envio v1.0` (scenario 7597712, 3 módulos: Search `Pendiente`+`sla_alerta_roja` → Gmail conn 9514506 → Update `id`+`record` con `estado_envio=Enviado`, `enviado_en={{now}}`, `intentos=1`). Run once responsive y stop inmediato: 2/2 filas `Enviado`, auditor ciego OK. Evidencia en `docs/_evidencia/T-MAKE-SLA-ENVIO-20260924/`.
**Prevención futura:** (1) antes de planear "agregar una rama al escenario X", confirmar por API que X existe, corre y tiene el trigger supuesto — `GET /scenarios/{id}/logs` distingue en segundos un escenario que trabaja de uno que nunca corrió; (2) los `grep` de un TABLE_ID sobre blueprints dan falsos positivos por metadata de interfaz (specs de linked fields): mirar el contexto del match, no el conteo.
**Inconveniente 2:** `POST /scenarios/{id}/run` sobre escenario inactivo devuelve `422 IM325 "Scenario is not activated"`. **Solución:** secuencia `start → run → stop`; con fórmula idempotente (solo toma Pendientes) una corrida programada colada es inocua. **Prevención futura:** para despachos one-shot desde API, planear el ciclo start/run/stop desde el diseño y dejar el escenario como estaba.

### 2026-09-25 — T-INFORME-ENSAMBLADOR · drift de parejas de campos en TX_ItemsCuadroValoracion y vara del score de paridad
**Contexto:** Tanda 1 del roadmap de paridad — construcción del ensamblador `InformeContexto` (`lib/informe/`) + matriz de tags + medidor, corriendo contra el caso real VP-2026-0066.
**Inconveniente:** el `totalUf` del modelo canónico (`lib/tasador/lectura-informe.ts`) daba 0 para 0066: las 6 filas reales de `TX_ItemsCuadroValoracion` pueblan la pareja "vieja" (`nombre_item`/`uf_m2_unitario`/fórmula `valor_uf`) y dejan vacía la pareja que el canónico lee (`descripcion`/`uf_m2_aplicado`/`uf_total_item`).
**Causa raíz:** el escritor del motor y el lector del canónico se construyeron en tandas distintas contra dos generaciones de columnas de la misma tabla, y nadie corrió el canónico contra un caso poblado por el motor hasta ahora.
**Solución aplicada:** el ensamblador (`lib/informe/ensamblador.ts`) relee los ítems con caída vieja→nueva, documentada y con test dedicado; el total sale 20.125,8624 UF = PDF de referencia. Sin tocar `lectura-informe.ts` (mandato de la tanda). Queda como candidato a fix de fondo en tanda posterior (alinear escritor o lector).
**Prevención futura:** cuando dos módulos comparten tabla pero se escribieron en tandas distintas, verificar contra un registro REAL poblado por el escritor antes de asumir que los nombres de campo coinciden; un test con fixture sintético inventado habría ocultado exactamente este drift.

**Inconveniente 2:** el score de paridad medido (34,1%) quedó bajo el 43% de la auditoría y parecía regresión. **Causa raíz:** cambio de vara — la auditoría contaba *caminos* OK; el medidor exige *valor presente*, y 0066 es un sandbox tipeado sin filas en TX_Comparables/Adjuntos/Habitaciones/Legales (26 E-ids OK sin dato). Medido por camino da 46,8%, en banda. **Solución aplicada:** criterio exacto documentado en el JSDoc de `lib/informe/medidor.ts` (score_datos, score_total_228 y score de camino) y número fijado en test. **Prevención futura:** todo "score" que se compare entre tandas debe declarar su denominador y su criterio de presencia; dos números con el mismo nombre y distinta vara generan falsas regresiones.

### 2026-09-25 — T-ARREGLOS-DOCS · la premisa de P1-6a no existía en la base y el DAG escribe filas, no columnas
**Contexto:** Tandas 2+3 del roadmap de paridad — dólar en CRON_UF_Diaria, "campo mal apuntado" (P1-6a), promedios de comparables al motor (P1-8) y saneo documental (P2-5).
**Inconveniente:** el fix instruido por el roadmap para P1-6a ("corregir `uso_campo_destino=sup_m2` → `sup_construccion_m2`") no tenía objeto: los únicos 2 registros de `D_TipoDocumentoAtributo` con ese valor (`recaDdzrS30jz7xNY`, `recycv6hnoK9krfxg`) apuntan a `TX_Unidades`, donde `sup_m2` SÍ existe — coincide con la matriz de `docs/schema-airtable.md:1720-1721`. Ejecutar el "fix" a ciegas habría roto el routing RF-09.
**Causa raíz:** la auditoría de paridad resumió el desalineamiento real (motor lee `TX_DatosTasacion.sup_construccion_m2`, RF-09 escribe `TX_Unidades.sup_m2` — eso es P1-6b/decisión A4) como si fuera un valor apuntando a columna inexistente, y el roadmap heredó esa formulación sin re-verificarla.
**Solución aplicada:** verificación MCP con filtro exacto antes de instruir nada; P1-6a cerrado "sin acción" con evidencia en `CIERRE_T-ARREGLOS-DOCS-20260925.md` §1; el frente queda subsumido en A4/P1-6b.
**Prevención futura:** todo fix de datos heredado de una auditoría anterior se re-verifica contra la base con el filtro exacto ANTES de escribir la instrucción manual — el costo es una query; el costo de no hacerlo es romper un flujo que funcionaba.

**Inconveniente 2:** el roadmap pedía "2 columnas destino en TX_Calculos" y "linkear F_UFm2_promedio a la regla"; el patrón real del DAG es otro (escribe una FILA por fórmula: `variable_output` → `resultado`, gateado por `formulas_resultado` de la regla) y la fórmula YA estaba linkeada a las 6 reglas activas — lo pendiente real era re-expresarla (`uf_m2_promedio_residencial_comuna` → `promedio_uf_m2_muestra` con fallback). **Causa raíz:** el plan se escribió con el modelo mental "columna por variable" sin contrastar el mecanismo de escritura del DAG. **Solución aplicada:** lector `sumComparables` aditivo y fail-safe (NO-guard, jamás aborta) en AT03 v11.2.0_v32b1; los pasos manuales instruyen editar la expresión de la fórmula, no crear columnas. **Prevención futura:** antes de planear escrituras del motor, mirar el Filtro de salida del DAG (qué escribe y cómo se gatea), no asumir esquema.

**Inconveniente 3:** `node --check` falla sobre `AT03_Calculos_DAG.js` — y no es un error del script: es cuerpo de función async del sandbox de Airtable (usa `await`/`return` top-level). **Solución aplicada:** validar sintaxis construyéndolo como `new AsyncFunction(codigo)` — el contexto real. **Prevención futura:** para scripts de Airtable scripting, el check sintáctico es AsyncFunction, no `node --check`; documentado también en el cierre de tanda.

### 2026-09-25 — T-APLICAR-AIRTABLE: saneo de datos vía REST (OAuth MCP caído) + typecast + denegación de batch
**Contexto:** aplicar el saneo de datos del punto 3 del CIERRE_T-ARREGLOS-DOCS (2 estados de C_AutomationsAirtable, 4 filas de inventario, VP-NaN-0066→VP-2026-0066, borrar seed demo, completar Z_EscenariosMake).

**Inconveniente 1 — el OAuth del MCP Airtable no completó.** El flujo devolvió `error=invalid_request` en el callback (PKCE: "code_challenge must be a valid url-encoding of the sha256 of the code_verifier") y el `state` llegó corrupto con `++`. Sin `code` no hay nada que canjear con `complete_authentication`, y el login interactivo de Airtable solo lo puede hacer el usuario en su navegador — Claude Code no tiene herramienta de navegador. **Causa raíz:** el generador PKCE del cliente OAuth de esta sesión emitió un challenge inválido. **Solución aplicada:** fallback RO-30 — REST API con `AIRTABLE_TOKEN` de `.env.local` (el mismo de producción, `patx…`, 82 chars), declarando el motivo. Todas las lecturas y escrituras del saneo salieron por curl/urllib. **Prevención futura:** cuando el OAuth del MCP falle, el fallback REST server-side es válido y suficiente para saneo de datos; no insistir en pedirle al usuario que "complete el navegador" si el callback trae `error=`.

**Inconveniente 2 — el estado "undeployed" no existía como opción del singleSelect.** El doc pedía marcar AT02/AT04 como "undeployed", pero `C_AutomationsAirtable.estado` solo tenía `[Inventariado, Activo, Pendiente, Inactivo]`. **Solución aplicada:** crear la opción `Undeployed` con `typecast:true` (autorizado por la tanda), fiel al literal del doc, + nota explicando el motivo. No se stopeó porque el estado ORIGINAL sí coincidía con el doc ("Activo") y typecast estaba permitido. **Prevención futura:** cuando el doc pide un valor de select que no existe, `typecast:true` lo crea en el PATCH/POST; verificar primero las choices reales vía meta API para decidir crear-opción vs mapear a existente.
