...
- **RO-43 · Un blueprint Make generado sin un import previo del mismo tipo de
  módulo no está verificado: el import es la prueba.** Al generar
  `SC-IApro-LeadsScraper.blueprint.json` (11-sep-2026), los blueprints del repo
  eran de otro dominio (Airtable/Dropbox/Anthropic), así que la única referencia
  verificada contra un import real fueron `http:ActionSendData` v3 y
  `util:SetVariable`/`SetVariables` v1. Los identificadores `module`/`version` de
  `google-sheets:*`, `apify:runActor`/`getDatasetItems`,
  `builtin:BasicFeeder`/`BasicRepeater`/`Sleep`, `datastore:AddReplaceRecord` y
  `email:ActionSendEmail` se escribieron con la nomenclatura pública de Make
  **sin cross-check contra un import previo** → riesgo «Module Not Found» al
  importar (mismo fallo que causó la reconstrucción de SC-RF09 v2.1 desde backup).
  **Regla:** cuando el repo no tenga un blueprint del mismo tipo de módulo para
  copiar literal, no dar por bueno el JSON generado; el import en Make es la
  verificación. Ante un módulo en rojo, corregir su `module`/`version` en el
  `.json` versionado (R1) y reimportar (R3) — nunca dejar el arreglo sólo en el
  canvas, porque no vuelve al repo y la próxima generación repite el error.
  Vigente desde el 11-sep-2026.
