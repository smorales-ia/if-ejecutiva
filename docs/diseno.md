...
> **Fuentes canónicas**: Blueprint Interfaces v2.10 §7.2 · Especificación v1.9.9 · Capa Datos v2.6.5
...
> **SLA: dos relojes, no uno (spec v1.9.9 §1.2 · §5.2).** El `SLABadge` y la vista 2 muestran
> el **plazo agregado** por par (cliente, tipo_informe), en días, sobre `C_SLA` (RN-04). Desde
> v1.9.9 la spec define además un **plazo por etapa** del workflow —siete etapas, en horas
...
> (CI-005) y su UI no está diseñada — cuando se aborde, no reutilizar el `SLABadge` agregado
> sin decidir antes cuál manda en cada superficie.
